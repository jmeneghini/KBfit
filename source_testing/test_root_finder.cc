...
#ifndef M_PI // Define M_PI if not defined (e.g., on MSVC without
             // _USE_MATH_DEFINES)
...
template <typename T> std::string vec_to_string(const std::vector<T>& vec) {
  ...
  for (const auto& val : vec) {
    ...
    oss << std::fixed << std::setprecision(10)
        << val; // Formatting within ostringstream is fine
    first = false;
  }
  oss << "]";
  return oss.str();
...
void check_roots(
    ...
    AdaptiveBracketRootFinder::EvalFn func_to_test, double a, double b,
    const std::vector<double>& expected_roots,
    const AdaptiveBracketConfig& config =
        AdaptiveBracketConfig(),       // Default config
    double root_comparison_tol = 1e-7, // Tolerance for comparing root values
    bool check_eval_count =
        false, // Optional: check eval count (for specific performance checks)
    size_t max_expected_evals =
        0) { // Renamed from check_eval_count to avoid conflict

  AdaptiveBracketRootFinder::EvalFn local_func =
      func_to_test; // EvalFn taken by non-const ref by finder
  ...
  oss_config_info << "Config: initial_step=" << config.initial_step_percent
                  ...
                  << ", min_step=" << config.min_step_percent
                  << ", max_step=" << config.max_step_percent
  ...
  if (found_roots.size() == expected_roots.size()) {
    for (size_t i = 0; i < expected_roots.size(); ++i) {
      ...
      oss_root_check << "Root mismatch at index " << i << ": expected "
                     ...
                     << ", found " << std::fixed << std::setprecision(10)
                     << found_roots[i] << ", difference: " << std::scientific
                     << (found_roots[i] - expected_roots[i]);
      CHECK_MESSAGE(std::abs(found_roots[i] - expected_roots[i]) <
                        root_comparison_tol,
                    oss_root_check.str());
    }
  }

  if (check_eval_count) { // check_eval_count is the bool flag,
                          // max_expected_evals is the value
...
TEST_SUITE("AdaptiveBracketRootFinder Tests") {
  AdaptiveBracketConfig
      defaultConfig; // Default constructor values from struct definition

  // function that takes in a vector of real functions, and computes Product[1 +
  // Exp[I fList[[i]][t]], {i, Length[fList]}]
  std::complex<double> product_exp(
      const std::vector<std::function<double(double)>>& fList, double t) {
  ...
  AdaptiveBracketRootFinder::EvalFn f_product_exp_1 = [](double t) {
  ...
  TEST_CASE("Complex Product Exponential: {f(t) = t, t/2}") {
    ...
    check_roots("Product Exp: (1+exp(i*pi*t))", f_product_exp_1, 0.5, 3.5,
                {1.0, 2.0, 3.0}, cfg);
  }

  AdaptiveBracketRootFinder::EvalFn f_product_exp_2 = [](double t) {
    std::vector<std::function<double(double)>> fList = {
  ...
  TEST_CASE("Complex Product Exponential w/ Plateau: {f(t) = -(t-5)^3, "
            "0.01*sin(8*(t-3))}") {
    AdaptiveBracketConfig cfg = defaultConfig;
    cfg.zero_tol = 1e-12;
    ...
    check_roots("Product Exp w/ Platuea: (1+exp(i*pi*f(t)))", f_product_exp_2,
                3.8, 6.2, {4.0, 6.0}, cfg);
  }

  AdaptiveBracketRootFinder::EvalFn f_product_exp_3 = [](double t) {
    std::vector<std::function<double(double)>> fList = {
  ...
  TEST_CASE("Complex Product Exponential w/ 4 functions: {f(t) = exp(-(t-1)), "
            "tan(t-3), exp(t-5)/10, -exp(-(t-3)^2)}") {
    AdaptiveBracketConfig cfg = defaultConfig;
    cfg.zero_tol = 1e-12;
    // cfg.initial_step_percent = 5e-2;
    // cfg.max_step_percent = 1e-1;
    // there's a touching root at 3.0, but in our situation the im part crosses
    // (at least thats what we thought, have to wait and see if we run into a
    // case where it doesn't)
    check_roots("Product Exp w/ 4 functions: (1+exp(i*pi*f(t)))",
                f_product_exp_3, 2.10, 3.90,
                {2.2146018366025516904, 3.7853981633974483096}, cfg);
  }
}
