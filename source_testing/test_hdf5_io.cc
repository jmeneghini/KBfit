...
public:
    ...
    void copyTo(unsigned int* buf) const { buf[0] = id; buf[1] = subid; }
...
TEST_SUITE("HDF5 I/O Tests") {
    ...
    TEST_CASE("IOMap HDF5 vs fstream compatibility") {
        ...
        const std::string header_content = "Test IOMap data for compatibility testing";
        ...
        SUBCASE("Write and Read fstream format") {
            ...
            iomap_write_fstream.openNew(fstream_file_raw, file_id, header_content, true, 'N', false, true, 'F');
            ...
            iomap_read_fstream.openReadOnly(fstream_file_raw, file_id, read_header_fstream, false, 'F');
        ...
        SUBCASE("Write and Read HDF5 format") {
            ...
            iomap_write_hdf5.openNew(hdf5_file_with_path, file_id, header_content, true, 'N', false, true, 'H');
            ...
            iomap_read_hdf5.openReadOnly(hdf5_file_with_path, file_id, read_header_hdf5, false, 'H');
        ...
        SUBCASE("Auto-detect file format and other IOMap operations") {
            ...
            if (std::filesystem::exists(fstream_file_raw)) std::filesystem::remove(fstream_file_raw);
            if (std::filesystem::exists(hdf5_file_raw)) std::filesystem::remove(hdf5_file_raw);

            IOMap<TestKey, std::vector<double>> iomap_f_w('F');
            iomap_f_w.openNew(fstream_file_raw, file_id, header_content, true, 'N', false, true, 'F');
            for (const auto& item : test_data) iomap_f_w.put(item.first, item.second);
            iomap_f_w.close();

            IOMap<TestKey, std::vector<double>> iomap_h_w('H');
            iomap_h_w.openNew(hdf5_file_with_path, file_id, header_content, true, 'N', false, true, 'H');
            for (const auto& item : test_data) iomap_h_w.put(item.first, item.second);
            iomap_h_w.close();

            // Test automatic format detection
            IOMap<TestKey, std::vector<double>> iomap_auto;
            std::string read_header_auto;
            iomap_auto.openReadOnly(fstream_file_raw, file_id, read_header_auto, false, 'D');
            iomap_auto.close();

            iomap_auto.openReadOnly(hdf5_file_with_path, file_id, read_header_auto, false, 'D');
            iomap_auto.close();
            ...
            for (int i = 0; i < 2; ++i) {
                for (int j = 0; j < 3; ++j) {
                    for (int k = 0; k < 2; ++k) {
                        ...
                        CHECK(read_array3d(i, j, k).real() == doctest::Approx(expected.real()));
                        CHECK(read_array3d(i, j, k).imag() == doctest::Approx(expected.imag()));
                    }
                }
            }
