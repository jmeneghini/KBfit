...
void doTests(XMLHandler& xml_rdr, int taskcount) {
  testMatrix(xml_rdr); /*
  ...
  testChisqDetRes(xml_rdr, taskcount); /*
  ...
  testMinimizer(xml_rdr);
  testKFitParamInfo(xml_rdr);
  testMatrixInverse(xml_rdr);
  // testNonIntRegulator(xml_rdr);
...
Tasker::Tasker() { TaskMap["KBFIT_TEST"] = &doTests; };

void Tasker::do_task(XMLHandler& xml_task, bool echo, int taskcount) {
  if (echo) {
    cout << "Input XML for this task:" << endl << xml_task.output() << endl;
  }
  ...
  if (taskit != TaskMap.end()) {
    (*(taskit->second))(xml_task, taskcount);
  } // do the task!!
  else {
...
int main(int argc, const char* argv[]) {
  cout << endl << "Starting KBFit tests" << endl;
  output_datetime();

  if (argc != 2) {
    cerr << "  KBFit testing requires one argument: "
...

  {
    ...
    if (xml_in.get_node_name() != "KBFit") {
      cerr << "  Root tag of input XML must be named \"KBFit\"" << endl;
      cerr << "  ... exiting..." << endl;
      return 1;
    }
    ...
    for (int task = 1; task <= ntasks; ++task) {
      ...
      try {
        ...
        if (xml_in.fail())
          throw(std::invalid_argument("XML input error"));
      ...
      swatch.stop();
      cout << "Task " << task
           << " done using time = " << swatch.getTimeInSeconds() << " secs"
           << endl;
      xml_in.seek_next_sibling();
    }
    ...
    rolex.stop();
    cout << endl << endl;
    cout << "KBFit testing: total time = " << rolex.getTimeInSeconds()
         << " secs" << endl;
  }
  cout << "KBFit testing: completion" << endl;
  output_datetime();
