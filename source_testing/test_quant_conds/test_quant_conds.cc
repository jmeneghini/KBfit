...
std::vector<std::complex<double>> hermitianMatrixToCVector(
...
std::vector<double> symmetricMatrixToVector(
        const RealSymmetricMatrix& M)
{
  const uint N = M.size();
...
TEST_SUITE("Quantization Conditions Tests") {
  ...
  TEST_CASE("Ktilde parameters can be set") {
    ...
    for (int i = 0; i < num_of_params; ++i) {
      ...
      CHECK(box_quant_inv->getParameterValue(kinvfitparams[i]) == param_vector[i]);
    }
  }
  ...
  TEST_CASE("Quantization conditions are correct") {
    ...
    cnpy::npy_save(file_dir + "B_from_inv.npy", B_from_inv_vec.data(),shape_inv, "w");
    cnpy::npy_save(file_dir + "Ktilde.npy", Ktilde_vec.data(),shape, "w");
    cnpy::npy_save(file_dir + "Ktilde_inv.npy", Ktilde_inv_vec.data(),shape_inv, "w");

    // run python script
    REQUIRE(std::system(("python3 " + file_dir + "test_quant_conds.py").c_str()) == 0);
    ...
    std::complex<double>* refStildeinv =
      reinterpret_cast<std::complex<double>*>(refStildeFromInv.data<double>());
    ...
    CB_ref_mat.putFromVector(std::vector<complex<double>>(refCBvec, refCBvec + B.size()*B.size()));
    Stilde_ref_mat.putFromVector(std::vector<complex<double>>(refStildevec, refStildevec + B.size()*B.size()));
    Stilde_inv_ref_mat.putFromVector(std::vector<complex<double>>(refStildeinv, refStildeinv + B_from_inv.size()*B_from_inv.size()));
    ...
    CHECK(TestHelper::approx_equal_cmatrix(Stilde_from_inv, Stilde_inv_ref_mat));
    ...
    for (int i = 0; i < 4; ++i) {
      BoxQuantization::QuantCondType qctype = static_cast<BoxQuantization::QuantCondType>(i);
      CMatrix Q(1, 1);
      CMatrix Q_ref(1, 1);
      ...
      for (int j = 0; j < evs.size(); ++j) {
        string message = "ev " + to_string(j)  + " failed for quant cond " + to_string(i);
        CHECK_MESSAGE(TestHelper::approx_equal_complex(evs_ref[stride_ind*evs.size() + j], evs[j]), message);
      }
      string message = "omega failed for quant cond " + to_string(i);
      CHECK_MESSAGE(TestHelper::approx_equal_complex(qc_omega_ref_vec[i], omega),
        message);
    }
  }
} // TEST_SUITE