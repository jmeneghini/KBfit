...
public:
  ...
  void
  seek_unique(const std::string& tagname); // starting root and all descendents

  void seek_unique_to_child(
      const std::string& tagname); // seek in root and children only

  void seek_unique_child(
      const std::string& tagname); // seek among children only (not root)
  ...
  std::list<XMLHandler>
  find_among_children(const std::list<std::string>& tagnames) const;

  int count_among_children(const std::string& tagname) const;

  int count_to_among_children(
      const std::string& tagname) const; // includes current
...
void primitive_extract_from_string(const std::string& pstring, T& result,
  ...
  try {
    ...
    err << "Error: Failed to convert string: \"" << pstring << "\" to "
        << ptype;
...
void primitive_vector_extract(const std::string& pstring,
  ...
  try {
    ...
    if ((!is.eof()) && is.fail()) {
      throw(std::invalid_argument(std::string("Error in reading vector ") +
                                  pstring));
    }
  } catch (const std::exception& xp) {
    std::ostringstream err;
    err << "Error: Failed to convert string: \"" << pstring
        << "\" to vector of " << ptype;
    throw(std::invalid_argument(err.str()));
  }
}
...
//  stringtuple has form  (1,3,-1)  (integers, no spaces)
