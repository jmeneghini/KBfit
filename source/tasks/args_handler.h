...
public:
  ...
  void echo(XMLHandler& xmlout) const { xmlout = m_xmlout; }

  std::string echo() const { return m_xmlout.output(); }

  void getInput(XMLHandler& xmlin) const { xmlin = m_xmlin; }

  std::string getInput() const { return m_xmlin.output(); }

  std::string getInputRootTag() const {
    m_xmlin.seek_root();
    return m_xmlin.get_node_name();
  }
  ...
  std::list<ArgsHandler> getSubHandlers(const std::string tagname) {
    ...
    for (std::list<XMLHandler>::iterator it = xmlsub.begin();
         it != xmlsub.end(); it++)
  ...
  void getScalar(const std::string& tagname, double& val) {
    get_basic_item(tagname, val, true);
  }
  ...
  template <typename T> void getItem(const std::string& tagname, T& val) {
    try {
      val = T(m_xmlin);
      m_xmlin.seek_root();
      if (m_echo) {
  ...
  //  "tagname" is used only when outputting error message if failure
  template <typename T> T getItem(const std::string& tagname) {
    try {
      T val(m_xmlin);
      m_xmlin.seek_root();
      if (m_echo) {
  ...
  void getMultiItems(const std::string& tagname, std::list<T>& vals) {
    try {
      ...
      m_xmlin.seek_root();
      for (std::list<XMLHandler>::iterator it = xmllist.begin();
           it != xmllist.end(); it++) {
        try {
          T val = T(*it);
          if (m_echo) {
  ...
  void getMultiStrings(const std::string& tagname,
...
private:
  void assign_with_choices(XMLHandler& xmlin,
                           const std::set<std::string>& roottags,
    ...
    for (it = roottags.begin(); it != roottags.end(); it++) {
      if (seek_to_child_only)
        ct = xmlt.count_to_among_children(*it);
      else
        ct = xmlt.count(*it);
      count += ct;
      if (ct == 1)
    ...
    if (count != 1)
      throw(std::invalid_argument("ArgsHandler construction failure: no single "
                                  "root tag amoung choices found"));
    if (!seek_to_child_only)
      xmlt.seek_unique(*rt);
    else
      xmlt.seek_unique_to_child(*rt);
    m_xmlin.set(xmlt);
  ...
  template <typename T> T get_basic_item(const std::string& tagname) {
    try {
      m_xmlin.seek_unique_to_child(tagname);
      std::string content = m_xmlin.get_text_content();
      m_xmlin.seek_root();
  ...
  void get_string_item(const std::string& tagname, std::string& val,
                       bool required) {
    try {
      m_xmlin.seek_unique_to_child(tagname);
  ...
  std::string get_string_item(const std::string& tagname, bool is_name) {
    try {
      m_xmlin.seek_unique_to_child(tagname);
      std::string val =
          m_xmlin.get_text_content(); // white space throws exception
      m_xmlin.seek_root();
      if (is_name) {
        val = tidyName(val);
        if (val.empty())
  ...
  void get_bool_item(const std::string& tagname, bool& val, bool required) {
    ...
    if (content == "true")
      ...
      throw(std::invalid_argument(error_msg(tagname, "Invalid boolean value")));
    if (m_echo)
      m_xmlout.put_child(tagname, content);
  }

  std::string error_msg(const std::string& tagname, const std::string& errmsg) {
    m_xmlin.seek_root();
    return std::string("Could not read Tag[") + tagname +
           std::string("] when input root tag is Tag[") +
           m_xmlin.get_node_name() + std::string("] Message: ") + errmsg;
  }
};
