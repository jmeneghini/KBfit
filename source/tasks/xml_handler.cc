...
XMLDoc::XMLDoc(const string& tagname, const string& text_content) {
  ...
  try {
    ...
    clear();
    throw(std::invalid_argument("Invalid XML root tag"));
  }
}
...
XMLDoc::XMLNode* XMLDoc::create_by_parsing(const string& xmlstr, size_t start,
  ...
  try {
    ...
    while (lastevent != none)
      parse_to_next_event(xmlstr, curr, stop, lastevent, ancestors,
                          lastchildren);
    if (!ancestors.empty())
      throw(std::invalid_argument("Invalid XML"));
    return top;
    ...
    destroy(top);
    throw(std::invalid_argument("Invalid XML or allocation error"));
  }
}
...
XMLDoc::XMLNode* XMLDoc::create_copy(XMLDoc::XMLNode* top) {
  ...
  while (!done) {
    if ((curr->firstchild != 0) && (!ascending)) {
      ...
      nodecopy->nextsibling =
          new XMLNode(curr->name, nodecopy->parent, curr->text);
...
void XMLDoc::clear() { destroy(root); }
...
void XMLDoc::find_next_xml_tag_event(const string& xmlstr, size_t start,
  ...
  while (notdone) {
    ...
    if ((type == startdecl) || (type == enddecl))
      throw(std::invalid_argument("Invalid XML"));
    if (type == endcom) {
      if (!incomment)
        throw(std::invalid_argument("Invalid XML"));
...
void XMLDoc::parse_to_next_event(const string& xmlstr, size_t& curr,
  ...
  if (lastevent == starttag) {
    if (nextevent == endempty) {
      ...
      if (lastchildren.top() == 0)
        ...
        lastchildren.top()->nextsibling = newtag;
  ...
  if (!textpassed.empty()) {
    ...
    if (lastchildren.top() == 0)
      ancestors.top()->firstchild = textnode;
    else
      lastchildren.top()->nextsibling = textnode;
...
size_t XMLDoc::get_declaration(const string& instr) {
  ...
  if (pos != 0)
    throw(std::invalid_argument(
        "Invalid XML: declaration must start at first character"));
  if (instr.length() < 8)
    throw(std::invalid_argument("Invalid XML declaration"));
  if ((instr[2] != 'x') || (instr[3] != 'm') || (instr[4] != 'l'))
    throw(std::invalid_argument("Invalid XML"));
...
ostream& XMLDoc::output(ostream& os, XMLNode* top, int initial_indent) const {
  ...
  if ((!declaration.empty()) && (curr == root))
    os << declaration;
  while (!done) {
    if (newline) {
      os << endl;
...
void XMLDoc::add_reference(XMLHandler* ref) { refset.insert(ref); }
...
XMLHandler::XMLHandler(const XMLHandler& xmlin, const std::string& newroottag,
  ...
  try {
    if (!seek_to_child_only)
      seek_unique(newroottag);
    else
      seek_unique_to_child(newroottag);
...
void XMLHandler::do_set(const XMLHandler& xmlin, bool subtree, bool makecopy) {
  ...
  if (newroot->text)
    throw(std::runtime_error("XMLHandler copy failed: current node is text "
                             "valued, cannot be root tag"));
  if ((makecopy) && (newroot != 0)) {
    ...
    if (exceptions)
      throw(std::invalid_argument("XML set failed"));
  }
}
  ...
  if (current) {
    ...
    while ((tagfinder.good()) && (ncount < 2)) {
      if ((!(tagfinder.current->text)) &&
          (tagfinder.current->name == tagname)) {
...
list<XMLHandler> XMLHandler::find(const list<string>& tagnames) const {
  ...
  while (tagfinder.good()) {
    if (!(tagfinder.current->text)) {
      for (list<string>::const_iterator it = tagnames.begin();
           it != tagnames.end(); it++) {
...
list<XMLHandler>
XMLHandler::find_among_children(const list<string>& tagnames) const {
  list<XMLHandler> found;
  XMLHandler tagfinder(*this, XMLHandler::pointer);
  tagfinder.set_exceptions_off();
  tagfinder.seek_first_child();
  ...
  while (tagfinder.good()) {
    ...
    if (ncount > 1)
      throw(std::invalid_argument(std::string("Multiple occurrences of ") +
                                  tagname +
                                  std::string(" when one or none required")));
    if (ischild)
      tagfinder.seek_next_sibling();
    else {
      tagfinder.seek_first_child();
...
bool XMLHandler::is_simple_element() const {
  return (current) && (!(current->text)) && (current->firstchild) &&
         (current->firstchild->text) &&
         (current->firstchild->firstchild == 0) &&
...
void XMLHandler::put_sibling(const XMLHandler& xmlin) {
  try {
    ...
    clear();
    throw(std::invalid_argument("put_sibling failed"));
  }
}
...
void XMLHandler::put_sibling(const string& tagname,
  ...
  try {
    if ((fail()) || (current == root))
      throw(std::invalid_argument("Error"));
    XMLDoc::XMLNode* branch =
        content->create_simple_tag(tagname, text_content, 0);
    content->connect_as_nextsibling(current, branch);
    seek_next_sibling();
  } catch (const std::exception& xp) {
    clear();
...
void XMLHandler::put_sibling_text_node(const string& text) {
  try {
    ...
    clear();
    throw(std::invalid_argument("put_sibling_text_node failed"));
  }
}
...
void XMLHandler::put_child(const XMLHandler& xmlin) {
  try {
    ...
    clear();
    throw(std::invalid_argument("put_child failed"));
  }
}
...
void XMLHandler::put_child_text_node(const string& text) {
  try {
    ...
    clear();
    throw(std::invalid_argument("put_child_text_node failed"));
  }
}
...
void XMLHandler::set_text_content(const string& in_text_content) {
  ...
  clear();
  throw(std::invalid_argument("set_text_content failed"));
}
...
void XMLHandler::rename_tag(const string& newtagname) {
  ...
  try {
    ...
    clear();
    throw(std::invalid_argument("rename tag failed"));
  }
}
...
void XMLHandler::erase_current_element() {
  try {
    ...
    clear();
    throw(std::invalid_argument("erase_current_element failed"));
  }
}
...
void XMLHandler::erase_text_node() {
  try {
    ...
    clear();
    throw(std::invalid_argument("erase_text_node failed"));
  }
}
...
int XMLContentComparer::node_cmp(xmlNodePtr a_node, xmlNodePtr b_node) {
  // compare node types first
  if (a_node->text < b_node->text) {
    return -1;
  } else if (a_node->text > b_node->text) {
    return 1;
  }
...
bool XMLContentComparer::operator()(xmlNodePtr a_node, xmlNodePtr b_node) {
...
string tidyName(const string& str) {
  ...
  if (start == len)
    return tmp;
  size_t stop = len - 1;
  while ((isspace(str[stop])) && (stop > start))
    stop--;
...
void xml_either_tag_assert(XMLHandler& xmlh, const std::string& tagname1,
  ...
  if (xml_tag_count(xmlh, tagname1) == 1) {
    xmlh.seek_unique(tagname1);
    return;
  } else if (xml_tag_count(xmlh, tagname2) == 1) {
    xmlh.seek_unique(tagname2);
    return;
  }
  cout << "Bad XML input:" << endl;
  cout << "Expected one <" << tagname1 << "> or <" << tagname2 << "> tag"
       << endl;
...
void xml_root_assert(XMLHandler& xmlh, const std::string& tagname,
  ...
  try {
    xmlh.seek_root();
    if (xmlh.get_node_name() != tagname)
      throw(std::invalid_argument("assert"));
  } catch (const std::exception& xp) {
    cout << "Bad XML input to " << infoname << endl;
...
void xml_cerr(XMLHandler& xmlh, const std::string& msg) { cout << msg << endl; }
...
void xmlformat(const string& roottag, const string& inlogstr,
               XMLHandler& xmlout) {
  ...
  while (logstr.size() > 0) {
    ...
    if (pos != string::npos) {
      oline = logstr.substr(0, pos);
      logstr.erase(0, pos + 1);
    } else {
      oline = logstr;
      logstr.clear();
    }
    if (oline.find("<") == string::npos) {
      if (oline.size() > maxlength)
        maxlength = oline.size();
      oline = "<o>: " + oline;
    }
    outlines.push_back(oline);
  }
  for (list<string>::iterator it = outlines.begin(); it != outlines.end();
       ++it) {
    if (it->substr(0, 4) == "<o>:") {
      it->resize(maxlength + 5, ' ');
      (*it) += ":</o>";
    }
  }
