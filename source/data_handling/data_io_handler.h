...
// * *
// *   Objects of these "put" handlers always assume an "updating" mode.
// Existing  *
...
// *   size of the data to be put does not exceed the size of the data already
// in  *
...
// * *
// *       -- a write(ioh, const V&) must be defined (ioh is an IOHandler
// object)  *
...
template <typename H, typename R, typename D> class DataGetHandlerSF {
...
public:
  DataGetHandlerSF(H& in_handler, const std::string& file_name,
                   const std::string& filetype_id, bool use_checksums = false);
...
DataGetHandlerSF<H, R, D>::DataGetHandlerSF(H& in_handler,
  ...
  XMLHandler xmlr;
  xmlr.set_from_string(headerxml);
  if (!handler.checkHeader(xmlr)) {
    throw(std::invalid_argument(
        std::string("Header string in file is \n") + headerxml +
...
void DataGetHandlerSF<H, R, D>::outputKeys(XMLHandler& xmlout) {
  ...
  std::set<R> keys;
  iomptr->getKeys(keys);
  for (typename std::set<R>::const_iterator kt = keys.begin(); kt != keys.end();
       kt++) {
    XMLHandler xmlk;
    kt->output(xmlk);
    XMLHandler xmlt("Key");
...
template <typename H, typename R, typename D> class DataPutHandlerSF {
    ...
    if (!(iomptr->isNewFile())) {
      ...
      if (!handler.checkHeader(xmlr)) {
        fail("Header string in file is \n" + header +
             "\n header info in file " + file_name +
             " does not match info in current Handler\n ");
      }
    }
  } catch (const std::exception& xp) {
    fail("could not open file " + file_name + " for writing: " + xp.what());
  }
}
...
void DataPutHandlerSF<H, R, D>::fail(const R& rkey) {
  /* std::cout << "DataPutHandlerSF could not insert requested
   record:"<<std::endl; std::cout << " File name: "<< iomptr->getFileName()
   <<std::endl; XMLHandler xmlout; xmlout.set_root("RecordKey"); XMLHandler
   xmlh; rkey.output(xmlh); xmlout.put_child(xmlh); std::cout <<
   xmlout.str()<<std::endl; std::cout << "...execution aborted..."<<std::endl;
   delete iomptr;
   exit(1); */
  XMLHandler xmlkey;
  rkey.output(xmlkey);
  throw(std::runtime_error(
      std::string("DataPutHandlerSF could not insert requested record: ") +
      xmlkey.str()));
}
...
template <typename H, typename R, typename D> class DataGetHandlerMF {
...
DataGetHandlerMF<H, R, D>::DataGetHandlerMF(
    H& in_handler, const std::set<std::string>& file_names,
    const std::string& filetype_id, bool use_checksums)
    : m_handler(in_handler), m_filetype_id(filetype_id),
      m_use_checksums(use_checksums) {
  for (std::set<std::string>::const_iterator it = file_names.begin();
...
template <typename H, typename R, typename D>
bool DataGetHandlerMF<H, R, D>::addFile(const std::string& file_name,
                                        const std::set<R>& keys_to_keep) {
  bool flag = true;
  try {
    ...
    for (typename std::list<DataGetHandlerSF<H, R, D>*>::iterator it =
      ...
      if (tidyString(file_name) == (*it)->getFileName()) {
        throw(std::invalid_argument(
            std::string("cannot addFile ") + file_name +
            std::string(" in DataGetHandlerMF since already open ")));
      }
    // create the single-file getter
    DataGetHandlerSF<H, R, D>* newget = 0;
    try {
      newget = new DataGetHandlerSF<H, R, D>(m_handler, file_name,
                                             m_filetype_id, m_use_checksums);
    ...
    // check that all keys in new file are different from all keys already
    // available
    for (typename std::list<DataGetHandlerSF<H, R, D>*>::iterator it =
             getptrs.begin();
         it != getptrs.end(); it++) {
      ...
      std::set<R> intersect;
      std::set_intersection(newkeys.begin(), newkeys.end(), rkeys.begin(),
                            rkeys.end(),
    ...
    std::cout << "Fatal error: addFile failed in DataGetHandlerMF: "
              << xp.what() << std::endl;
    exit(1);
  }
  return flag;
...
std::set<std::string> DataGetHandlerMF<H, R, D>::getFileNames() const {
  ...
  for (typename std::list<DataGetHandlerSF<H, R, D>*>::const_iterator it =
           getptrs.begin();
       it != getptrs.end(); it++)
...
void DataGetHandlerMF<H, R, D>::getData(const R& rkey, D& result) {
  for (typename std::list<DataGetHandlerSF<H, R, D>*>::iterator it =
           getptrs.begin();
       it != getptrs.end(); it++)
    if ((*it)->getDataMaybe(rkey, result))
      return;
  XMLHandler xmlk;
  rkey.output(xmlk);
...
void DataGetHandlerMF<H, R, D>::getFileMap(XMLHandler& xmlout) {
  ...
  for (typename std::list<DataGetHandlerSF<H, R, D>*>::iterator it =
    ...
    for (typename std::set<R>::iterator et = keys.begin(); et != keys.end();
         et++) {
