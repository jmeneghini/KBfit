...
template <typename K, typename V> class IOFSTRMap : public IOMapBase<K, V> {
...
void IOFSTRMap<K, V>::openReadOnly(const std::string& filename,
  ...
  check_for_failure(ioh.isOpen(),
                    "Please do not open an already open IOFSTRMap!");
...
void IOFSTRMap<K, V>::openNew(const std::string& filename,
  ...
  check_for_failure(ioh.isOpen(),
                    "Please do not open an already open IOFSTRMap!");
  OpenMode mode = (fail_if_exists) ? IOFSTRHandler::ReadWriteFailIfExists
                                   : IOFSTRHandler::ReadWriteEraseIfExists;
...
void IOFSTRMap<K, V>::initialize(const std::string& filename, OpenMode mode,
  ...
  if (ioh.isNewFile()) { // this is a new file
    if (verbose1)
      std::cout << "IOFSTRMap: opening new file " << filename << std::endl;
    ...
    if (verbose1)
      std::cout << "IOFSTRMap: opening already existing file " << filename
                << std::endl;
    ioh.rewind();
...
template <typename K, typename V> void IOFSTRMap<K, V>::close() {
  if (!ioh.isOpen())
    return;
  if (write_map_on_close)
    writeMap(new_write_pos);
  if (verbose1)
    std::cout << "IOFSTRMap: closed file " << ioh.getFileName() << std::endl;
  ioh.close();
  file_map.clear();
}

template <typename K, typename V> void IOFSTRMap<K, V>::flush() {
  if (!ioh.isOpen())
    return;
  if (write_map_on_close)
    writeMap(new_write_pos);
  if (verbose1)
    std::cout << "IOFSTRMap: flushed file " << ioh.getFileName() << std::endl;
  write_map_on_close = false;
}
...
void IOFSTRMap<K, V>::readMap(w_pos mapstart) {
  check_for_failure(mapstart == 0,
                    "IOFSTRMap file " + ioh.getFileName() +
...
template <typename K, typename V> void IOFSTRMap<K, V>::setHighVerbosity() {
  verbose1 = verbose2 = true;
}

template <typename K, typename V> void IOFSTRMap<K, V>::setMediumVerbosity() {
  verbose1 = true;
  verbose2 = false;
}
...
template <typename K, typename V> std::string IOFSTRMap<K, V>::getHeader() {
...
void IOFSTRMap<K, V>::put(const K& key, const V& val) {
  ...
  if (flag) {
    ...
    if (verbose2)
      std::cout
          << "IOFSTRMap::put overwriting existing record at file location "
          << start_write_pos << std::endl;
  } else {
    if (verbose2)
      std::cout << "IOFSTRMap::put of new record at file location "
                << start_write_pos << std::endl;
    write_map_on_close = true;
...
void IOFSTRMap<K, V>::get(const K& key, V& val) {
  ...
  if (verbose2)
    std::cout << "IOFSTRMap::get at file location " << start << std::endl;
