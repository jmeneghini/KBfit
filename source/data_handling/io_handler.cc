...
const IOHandler::pos_type IOHandler::data_start_pos =
    1 + IOHandler::ID_string_length;
...
void IOHandler::check_for_failure(int errcode, const std::string& mesg) {
  if (errcode == IOHandler::IO_SUCCESS)
    return;
  std::cout << "IOHandler error with file " << m_filename << ":" << endl
            << "  " << mesg << endl;
  exit(errcode);
}
...
void IOHandler::open(const std::string& filename, IOHandler::OpenMode mode,
  ...
  if (exists) {
    IOHandler::openmode_type access =
...
void IOHandler::open_new_file(const std::string& filetype_id, char endianness) {
...
IOHandler::pos_type IOHandler::tell() { return file_tell() - data_start_pos; }
...
void IOHandler::turnOffChecksum() { checksum_on = false; }

void IOHandler::resetChecksum() { checksum = 0; }
...
void IOHandler::write_common(const char* output, size_t elementbytes,
                             size_t nelements) {
  if (!openflag)
...
void IOHandler::read_common(char* input, size_t elementbytes,
                            size_t nelements) {
  if (!openflag)
...
void IOHandler::write(const char& output) { write_basic<char>(output); }

void IOHandler::write(const int& output) { write_basic<int>(output); }

void IOHandler::write(const unsigned int& output) {
  write_basic<unsigned int>(output);
}

void IOHandler::write(const long int& output) { write_basic<long int>(output); }
...
void IOHandler::write(const float& output) { write_basic<float>(output); }

void IOHandler::write(const double& output) { write_basic<double>(output); }

void IOHandler::write(const bool& output) { write_basic<bool>(output); }

void IOHandler::write(const fcmplx& output) { write_complex<float>(output); }

void IOHandler::write(const dcmplx& output) { write_complex<double>(output); }
...
void IOHandler::read(char& input) { read_basic<char>(input); }

void IOHandler::read(int& input) { read_basic<int>(input); }

void IOHandler::read(unsigned int& input) { read_basic<unsigned int>(input); }

void IOHandler::read(long int& input) { read_basic<long int>(input); }

void IOHandler::read(unsigned long int& input) {
  read_basic<unsigned long int>(input);
}

void IOHandler::read(long long int& input) { read_basic<long long int>(input); }

void IOHandler::read(unsigned long long int& input) {
  read_basic<unsigned long long int>(input);
}

void IOHandler::read(float& input) { read_basic<float>(input); }

void IOHandler::read(double& input) { read_basic<double>(input); }

void IOHandler::read(bool& input) { read_basic<bool>(input); }

void IOHandler::read(fcmplx& input) { read_complex<float>(input); }

void IOHandler::read(dcmplx& input) { read_complex<double>(input); }

void IOHandler::read(std::vector<char>& input) {
  read_basic_vector<char>(input);
}

void IOHandler::read(std::vector<int>& input) { read_basic_vector<int>(input); }
