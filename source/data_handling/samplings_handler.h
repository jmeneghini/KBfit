...
public:
  SamplingsGetHandler(const MCBinsInfo& binfo, const MCSamplingInfo& sampinfo,
    ...
    m_get = new DataGetHandlerMF<SamplingsGetHandler, MCObsInfo,
                                 std::vector<double>>(
        *this, file_names, std::string("Sigmond--SamplingsFile"),
        use_checksums);
  }

  void addFile(const std::string& file_name) { m_get->addFile(file_name); }
  ...
  void clear() { m_get->clear(); }

  void close() { m_get->close(); }
  ...
  bool queryData(const MCObsInfo& rkey) const { return m_get->queryData(rkey); }

  void getData(const MCObsInfo& rkey, Vector<double>& result) const {
    std::vector<double> buffer;
    m_get->getData(rkey, buffer);
  ...
  std::set<MCObsInfo> getKeys() const { return m_get->getKeys(); }

  void outputKeys(XMLHandler& xmlout) const { m_get->outputKeys(xmlout); }

  void getFileMap(XMLHandler& xmlout) const { m_get->getFileMap(xmlout); }

  std::set<std::string> getFileNames() const { return m_get->getFileNames(); }

  unsigned int size() const { return m_get->size(); }

  bool checkHeader(XMLHandler& xmlin) {
    try {
      XMLHandler xmlb(xmlin, "SigmondSamplingsFile");
      MCBinsInfo bchk(xmlb);
      MCSamplingInfo schk(xmlb, bchk);
...
public:
  ...
  void putData(const MCObsInfo& rkey, const Vector<double>& data) {
    if (data.size() != m_samp_info.getNumberOfReSamplings() + 1)
      throw(std::runtime_error(
          "Wrong number of Samplings in SamplingsPutHandler::putData"));
    try {
      m_put->putData(rkey, data.c_vector());
    } catch (std::exception& xp) {
      throw(std::runtime_error(
          std::string("putData failed: permission problem or record already in "
                      "file and no overwrite -- ") +
          xp.what()));
    }
  }

  void flush() { m_put->flush(); }
