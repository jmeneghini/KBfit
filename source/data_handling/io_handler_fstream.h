...
// *   between big-endian and little-endian format, and it allows the user to
// turn *
// *   check sums on or off.  Files written by this class will start with a
// single *
// *   character 'B' or 'L' to indicate endian-ness.  Then an ID string is
// output, *
// *   which always has the length "ID_string_length"=32 (spaces are padded if *
// *   needed).  Files read by this class expect this starting behavior.  An
// error *
// *   occurs if the ID string given in the open command on an existing file
// does  *
...
//  stored in contiguous memory, and this class uses this fact.  A general
//  "struct" may or may not be stored in contiguous memory; padding characters
//  could be inserted between the struct elements (the GNU compiler
//  allows an "__attribute__ ((packed))" to prevent such padding).
//  This is why we must treat complex numbers differently than basic floats,
//  etc.
...
public:
  enum OpenMode {
    ReadOnly,
    ReadWriteFailIfExists,
    ReadWriteEraseIfExists,
...
private:
  ...
  template <bool b> void static__assert() { typedef char asserter[b ? 1 : -1]; }
#endif

  // private write members

  template <typename T> void write_basic(const T& output);

  template <typename T> void write_complex(const std::complex<T>& output);

  template <typename T> void multi_write_basic(const T* output, int n);

  template <typename T>
  void multi_write_complex(const std::complex<T>* output, int n);

  template <typename T> void write_basic_vector(const std::vector<T>& output);

  template <typename T>
  void write_complex_vector(const std::vector<std::complex<T>>& output);

  template <typename T> void write_array(const Array<T>& output);

  // private read members

  template <typename T> void read_basic(T& output);

  template <typename T> void read_complex(std::complex<T>& output);

  template <typename T> void multi_read_basic(T* output, int n);

  template <typename T> void multi_read_complex(std::complex<T>* output, int n);

  template <typename T> void read_basic_vector(std::vector<T>& output);

  template <typename T>
  void read_complex_vector(std::vector<std::complex<T>>& output);

  template <typename T> void read_array(Array<T>& output);

  // private numbytes members

  template <typename T> size_t numbytes_basic(const T& data) const;
  ...
  template <typename T>
  size_t
  numbytes_complex_vector(const std::vector<std::complex<T>>& data) const;

  template <typename T> size_t numbytes_array(const Array<T>& data) const;
};
...
template <typename T> void IOFSTRHandler::write_basic(const T& output) {
...
template <typename T>
void IOFSTRHandler::write_complex_vector(
...
template <typename T> void IOFSTRHandler::write_array(const Array<T>& output) {
...
template <typename T> void IOFSTRHandler::read_basic(T& input) {
  read_common((char*)&input, sizeof(T), 1);
}

template <typename T> void IOFSTRHandler::read_complex(std::complex<T>& input) {
  T data[2];
  read_common((char*)&data, sizeof(T), 2);
  input = std::complex<T>(data[0], data[1]);
}

template <typename T> void IOFSTRHandler::multi_read_basic(T* input, int n) {
...
void IOFSTRHandler::read_basic_vector(std::vector<T>& input) {
  ...
  check_for_failure((n < 0) || (n > 16777216),
                    "vector too large...bad file location?");
  input.resize(n);
...
template <typename T> void IOFSTRHandler::read_array(Array<T>& input) {
  ...
  // so place a reasonable limit
  check_for_failure(
      n > 128, "Array number of dimensions too large...bad file location?");
  read(input.m_sizes);
  read(input.m_store);
}
...
template <typename T>
size_t IOFSTRHandler::numbytes_complex_vector(
    const std::vector<std::complex<T>>& data) const {
...
inline void write(IOFSTRHandler& ioh, const char& output) { ioh.write(output); }

inline void write(IOFSTRHandler& ioh, const int& output) { ioh.write(output); }
...
inline void write(IOFSTRHandler& ioh, const bool& output) { ioh.write(output); }
...
inline void write(IOFSTRHandler& ioh,
...
inline void read(IOFSTRHandler& ioh, std::string& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, char& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, int& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, unsigned int& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, long int& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, unsigned long int& input) {
  ioh.read(input);
}

inline void read(IOFSTRHandler& ioh, long long int& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, unsigned long long int& input) {
  ioh.read(input);
}

inline void read(IOFSTRHandler& ioh, float& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, double& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, bool& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, fcmplx& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, dcmplx& input) { ioh.read(input); }
...
inline void read(IOFSTRHandler& ioh, Array<char>& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, Array<int>& input) { ioh.read(input); }
...
inline void read(IOFSTRHandler& ioh, Array<float>& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, Array<double>& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, Array<fcmplx>& input) { ioh.read(input); }

inline void read(IOFSTRHandler& ioh, Array<dcmplx>& input) { ioh.read(input); }
...
inline size_t numbytes(IOFSTRHandler& ioh,
...
inline size_t numbytes(IOFSTRHandler& ioh,
                       const std::vector<unsigned long int>& data) {
  return ioh.numbytes(data);
}
...
inline size_t numbytes(IOFSTRHandler& ioh,
                       const Array<unsigned long int>& data) {
  return ioh.numbytes(data);
}
...
// Vector<T> I/O functions - Vector is KBfit's custom vector class
template <typename T>
inline void write(IOFSTRHandler& ioh, const Vector<T>& output) {
  ioh.write(output.c_vector());
}

template <typename T> inline void read(IOFSTRHandler& ioh, Vector<T>& input) {
  std::vector<T> temp;
  ioh.read(temp);
  input = Vector<T>(temp);
}
...
template <typename T>
inline size_t numbytes(IOFSTRHandler& ioh, const Vector<T>& data) {
  return ioh.numbytes(data.c_vector());
}
