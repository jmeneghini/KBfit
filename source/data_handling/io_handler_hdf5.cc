...
const IOHDF5Handler::openmode_type IOHDF5Handler::IO_MODE_RDWR =
    ios::in | ios::out;
...
IOHDF5Handler::IOHDF5Handler()
    : fid(-1), is_new_file(false), read_only(true), openflag(false),
...
IOHDF5Handler::IOHDF5Handler(const std::string& filename,
...
void IOHDF5Handler::open(const std::string& filename,
                         IOHDF5Handler::OpenMode mode,
                         const std::string& filetype_id, char endianness,
                         bool turn_on_checksum) {
  close();
...
void IOHDF5Handler::open(const std::string& filename, ios_base::openmode mode,
  ...
  if (!(mode & ios_base::out))
    ...
    if (mode & ios_base::trunc)
      iomode = ReadWriteEraseIfExists;
    else
      iomode = ReadWriteFailIfExists;
  }
...
void IOHDF5Handler::open_existing_file(const std::string& filetype_id) {
  if (read_only) {
    ...
    check_for_hid_failure(fid,
                          "Could not open file for update reading/writing");
  }
  ...
  if ((endian_format != 'B') && (endian_format != 'L')) {
    flag = false;
    std::cerr << "Invalid endian format" << std::endl;
  }
  if (tidyString(filetype_id) != ID_string) {
    ...
    std::cerr << "ID requested was: <" << tidyString(filetype_id) << ">"
              << std::endl;
  }
  if (!flag)
...
void IOHDF5Handler::open_new_file(const std::string& filetype_id,
  ...
  check_for_hid_failure(
      fid, "Could not open file for reading/writing if nonexisting");
  openflag = true;
...
IOHDF5Handler::~IOHDF5Handler() { clear(); }
...
void IOHDF5Handler::mkdir(const std::string& path) {
  ...
  for (uint i = 0; i < tokens.size(); ++i) {
    ppath += tokens[i];
    if (!(queryDir(ppath))) {
      hid_t gid = H5Gcreate2(*cwdptr, ppath.c_str(), H5P_DEFAULT, H5P_DEFAULT,
                             H5P_DEFAULT);
...
void IOHDF5Handler::collect_data_names(
  ...
  if (info.type == H5O_TYPE_DATASET) {
    ...
    collected_names.insert(newpath);
    return;
  }
  if (info.type == H5O_TYPE_GROUP) {
    ...
    for (hsize_t i = 0; i < ginfo.nlinks; i++) {
      ssize_t nsize = H5Lget_name_by_idx(loc_id, ".", H5_INDEX_NAME,
                                         H5_ITER_INC, i, NULL, 0, H5P_DEFAULT) +
                      1;
      char* nextname = new char[nsize];
      nsize = H5Lget_name_by_idx(loc_id, ".", H5_INDEX_NAME, H5_ITER_INC, i,
...
void IOHDF5Handler::collect_data_names_in_currdir(
    hid_t loc_id, set<std::string>& collected_names,
    H5O_type_t query_type) const {
  H5G_info_t ginfo;
...
bool IOHDF5Handler::peekString(std::string& stringvalue,
...
bool IOHDF5Handler::peekID(std::string& stringvalue,
...
void IOHDF5Handler::turnOffChecksum() { checksum_on = false; }

void IOHDF5Handler::resetChecksum() { checksum = 0; }

ByteHandler::n_uint32_t IOHDF5Handler::getChecksum() {
  check_for_failure(
      !checksum_on,
      "Invalid call to getChecksum since checksums not turned on");
  return checksum;
}
...
void IOHDF5Handler::printFileID() {
  ...
  std::cout << "File = " << m_filename << std::endl
            << "ID string = <" << tidyString(ID_string) << ">" << std::endl;
}
...
void IOHDF5Handler::check_for_failure(int errcode,
                                      const std::string& mesg) const {
  if (errcode == IOHDF5Handler::IO_SUCCESS)
    return;
  std::cout << "IOHDF5Handler error with file " << m_filename << ":" << endl
            << "  " << mesg << endl;
  exit(errcode);
}

void IOHDF5Handler::check_for_hid_failure(hid_t id,
                                          const std::string& mesg) const {
  if (id < 0) {
    std::cout << "IOHDF5Handler error with file " << m_filename << ":" << endl
              << "  " << mesg << endl;
...
void IOHDF5Handler::check_for_herr_failure(herr_t status,
                                           const std::string& mesg) const {
  if (status < 0) {
    std::cout << "IOHDF5Handler error with file " << m_filename << ":" << endl
              << "  " << mesg << endl;
...
void IOHDF5Handler::check_token(const std::string& token, bool allowdot,
  ...
  //    check_for_failure(IO_ERR_OTHER," blank spaces in object names not
  //    allowed here!!");
  ...
  for (uint i = 0; i < token.length(); ++i) {
    if ((iswspace(token[i])) && (token[i] != ' ')) {
      check_for_failure(
          IO_ERR_OTHER,
          " white space other than blanks not allowed in object names!!");
    }
  }
}
...
std::string IOHDF5Handler::form_absolute_path(
    const std::vector<std::string>& path_links) const {
  if (path_links.empty())
    return std::string("/");
  std::string result = "";
  for (std::vector<std::string>::const_iterator it = path_links.begin();
       it != path_links.end(); ++it)
    result += "/" + (*it);
  return result;
}
...
void IOHDF5Handler::write(const std::string& objname,
  ...
  herr_t status =
      H5Dwrite(dataid, typid, H5S_ALL, H5S_ALL, H5P_DEFAULT, output.c_str());
  ...
  if (checksum_on)
    checksum =
        m_bytehandler.get_checksum(checksum, output.c_str(), output.length());
}
...
void IOHDF5Handler::write(const std::string& objname,
  ...
  write_atomics<unsigned long int>(objname, &output, 1, dtype_ulong,
                                   H5T_NATIVE_ULONG);
}

void IOHDF5Handler::write(const std::string& objname,
                          const long long int& output) {
  write_atomics<long long int>(objname, &output, 1, dtype_llong,
                               H5T_NATIVE_LLONG);
}

void IOHDF5Handler::write(const std::string& objname,
                          const unsigned long long int& output) {
  write_atomics<unsigned long long int>(objname, &output, 1, dtype_ullong,
                                        H5T_NATIVE_ULLONG);
}
...
void IOHDF5Handler::write(const std::string& objname, const fcmplx& output) {
  write_complex_values<float>(objname, &output, 1, dtype_float,
                              H5T_NATIVE_FLOAT);
}

void IOHDF5Handler::write(const std::string& objname, const dcmplx& output) {
  write_complex_values<double>(objname, &output, 1, dtype_double,
                               H5T_NATIVE_DOUBLE);
}

void IOHDF5Handler::write(const std::string& objname, const bool& output) {
  write_atomics<bool>(objname, &output, 1, dtype_int, H5T_NATIVE_INT);
}
...
void IOHDF5Handler::write(const std::string& objname,
  ...
  write_atomics<int>(objname, output.data(), output.size(), dtype_int,
                     H5T_NATIVE_INT);
}

void IOHDF5Handler::write(const std::string& objname,
                          const std::vector<unsigned int>& output) {
  write_atomics<unsigned int>(objname, output.data(), output.size(), dtype_uint,
                              H5T_NATIVE_UINT);
}

void IOHDF5Handler::write(const std::string& objname,
                          const std::vector<long int>& output) {
  write_atomics<long int>(objname, output.data(), output.size(), dtype_long,
                          H5T_NATIVE_LONG);
}

void IOHDF5Handler::write(const std::string& objname,
                          const std::vector<unsigned long int>& output) {
  write_atomics<unsigned long int>(objname, output.data(), output.size(),
                                   dtype_ullong, H5T_NATIVE_ULLONG);
}

void IOHDF5Handler::write(const std::string& objname,
                          const std::vector<float>& output) {
  write_atomics<float>(objname, output.data(), output.size(), dtype_float,
                       H5T_NATIVE_FLOAT);
}

void IOHDF5Handler::write(const std::string& objname,
                          const std::vector<double>& output) {
  write_atomics<double>(objname, output.data(), output.size(), dtype_double,
                        H5T_NATIVE_DOUBLE);
}

void IOHDF5Handler::write(const std::string& objname,
                          const std::vector<fcmplx>& output) {
  write_complex_values<float>(objname, output.data(), output.size(),
                              dtype_float, H5T_NATIVE_FLOAT);
}

void IOHDF5Handler::write(const std::string& objname,
                          const std::vector<dcmplx>& output) {
  write_complex_values<double>(objname, output.data(), output.size(),
                               dtype_double, H5T_NATIVE_DOUBLE);
}

void IOHDF5Handler::write(const std::string& objname,
                          const Array<int>& output) {
  write_array<int>(objname, output, dtype_int, H5T_NATIVE_INT);
}

void IOHDF5Handler::write(const std::string& objname,
                          const Array<unsigned int>& output) {
  write_array<unsigned int>(objname, output, dtype_uint, H5T_NATIVE_UINT);
}

void IOHDF5Handler::write(const std::string& objname,
                          const Array<long int>& output) {
  write_array<long int>(objname, output, dtype_long, H5T_NATIVE_LONG);
}

void IOHDF5Handler::write(const std::string& objname,
                          const Array<unsigned long int>& output) {
  write_array<unsigned long int>(objname, output, dtype_ulong,
                                 H5T_NATIVE_ULONG);
}

void IOHDF5Handler::write(const std::string& objname,
                          const Array<float>& output) {
...
void IOHDF5Handler::read(const std::string& objname, std::string& input) {
  ...
  herr_t status =
      H5Dread(dataset_id, nat_type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, buffer);
  ...
  if (checksum_on)
    checksum =
        m_bytehandler.get_checksum(checksum, input.c_str(), input.length());
}
...
void IOHDF5Handler::read(const std::string& objname,
                         unsigned long long int& input) {
  read_atomic<unsigned long long int>(objname, input, dtype_ullong,
                                      H5T_NATIVE_ULLONG);
}
...
void IOHDF5Handler::read(const std::string& objname, bool& input) {
  read_atomic<bool>(objname, input, dtype_int, H5T_NATIVE_INT);
}
...
void IOHDF5Handler::read(const std::string& objname,
                         std::vector<unsigned long int>& input) {
  read_atomics<unsigned long int>(objname, input, dtype_ulong,
                                  H5T_NATIVE_ULONG);
}
