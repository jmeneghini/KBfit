...
const IOFSTRHandler::pos_type IOFSTRHandler::data_start_pos =
    1 + IOFSTRHandler::ID_string_length;
...
const IOFSTRHandler::openmode_type IOFSTRHandler::IO_MODE_RDWR =
    ios::in | ios::out;
...
void IOFSTRHandler::check_for_failure(int errcode, const std::string& mesg) {
  if (errcode == IOFSTRHandler::IO_SUCCESS)
    return;
  std::cout << "IOFSTRHandler error with file " << m_filename << ":" << endl
            << "  " << mesg << endl;
  exit(errcode);
}
...
IOFSTRHandler::IOFSTRHandler()
    : read_only(true), openflag(false), read_mode(true), endian_format('U'),
      endian_convert(false), checksum_on(false), is_new_file(false),
      checksum(0) {
#ifndef NO_CXX11
...
IOFSTRHandler::IOFSTRHandler(const std::string& filename,
...
void IOFSTRHandler::open(const std::string& filename,
                         IOFSTRHandler::OpenMode mode,
                         const std::string& filetype_id, char endianness,
                         bool turn_on_checksum) {
  close();
  ...
  if (exists) {
    IOFSTRHandler::openmode_type access =
...
void IOFSTRHandler::open_existing_file(
    const std::string& filetype_id, IOFSTRHandler::openmode_type access_mode) {
...
bool IOFSTRHandler::peekString(std::string& stringvalue,
...
bool IOFSTRHandler::peeker(std::string& stringvalue, unsigned int byte_offset,
  ...
  if (endian_convert)
    m_bytehandler.byte_swap(&n, sizeof(int), 1);
  if (n > 16777216)
    return false; // too large for string...must be corrupt data
...
bool IOFSTRHandler::peekID(std::string& stringvalue,
                           const std::string& filename) {
  stringvalue.clear();
  string fname = tidyString(filename);
  if (fname.empty())
...
void IOFSTRHandler::turnOffChecksum() { checksum_on = false; }

void IOFSTRHandler::resetChecksum() { checksum = 0; }
...
void IOFSTRHandler::write_common(const char* output, size_t elementbytes,
                                 size_t nelements) {
  if (!openflag)
  ...
  if (endian_convert) {
    ...
    if (checksum_on)
      checksum = m_bytehandler.get_checksum(checksum, output,
                                            elementbytes * nelements);
...
void IOFSTRHandler::read_common(char* input, size_t elementbytes,
                                size_t nelements) {
  if (!openflag)
  ...
  if (checksum_on)
    checksum =
        m_bytehandler.get_checksum(checksum, input, elementbytes * nelements);
  if (endian_convert)
    m_bytehandler.byte_swap(input, elementbytes, nelements);
}
...
void IOFSTRHandler::write(const char& output) { write_basic<char>(output); }

void IOFSTRHandler::write(const int& output) { write_basic<int>(output); }
...
void IOFSTRHandler::write(const float& output) { write_basic<float>(output); }

void IOFSTRHandler::write(const double& output) { write_basic<double>(output); }

void IOFSTRHandler::write(const bool& output) { write_basic<bool>(output); }
...
void IOFSTRHandler::read(std::string& input) {
  ...
  check_for_failure((str == 0),
                    "IOFSTRHandler::read---unable to allocate memory");
  readCommon(str, sizeof(char) * n);
  if (checksum_on)
    checksum = m_bytehandler.get_checksum(checksum, str, sizeof(char) * n);
  input.assign(str, n);
  delete[] str;
}

void IOFSTRHandler::read(char& input) { read_basic<char>(input); }

void IOFSTRHandler::read(int& input) { read_basic<int>(input); }

void IOFSTRHandler::read(unsigned int& input) {
  read_basic<unsigned int>(input);
}

void IOFSTRHandler::read(long int& input) { read_basic<long int>(input); }
...
void IOFSTRHandler::read(float& input) { read_basic<float>(input); }

void IOFSTRHandler::read(double& input) { read_basic<double>(input); }

void IOFSTRHandler::read(bool& input) { read_basic<bool>(input); }

void IOFSTRHandler::read(fcmplx& input) { read_complex<float>(input); }

void IOFSTRHandler::read(dcmplx& input) { read_complex<double>(input); }
...
void IOFSTRHandler::read(Array<char>& input) { read_array<char>(input); }

void IOFSTRHandler::read(Array<int>& input) { read_array<int>(input); }
...
void IOFSTRHandler::read(Array<float>& input) { read_array<float>(input); }

void IOFSTRHandler::read(Array<double>& input) { read_array<double>(input); }

void IOFSTRHandler::read(Array<fcmplx>& input) { read_array<fcmplx>(input); }

void IOFSTRHandler::read(Array<dcmplx>& input) { read_array<dcmplx>(input); }
...
size_t
IOFSTRHandler::numbytes(const std::vector<unsigned long int>& data) const {
