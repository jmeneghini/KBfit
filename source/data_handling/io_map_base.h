...
template <typename K, typename V> class IOMapBase {
...
public:
  ...
  virtual void get(const K& key,
                   V& val) = 0; // throws exception or aborts if fails

  virtual bool get_maybe(const K& key,
                         V& val) = 0; // returns false is fails, true otherwise

  virtual bool exist(const K& key) const = 0;

  virtual void
  flush() = 0; // puts file in finalized state so no data loss if abort occurs
  ...
  keepKeys(const std::set<K>&
               keys_to_keep) = 0; // keep only those keys in "keys_to_keep"
