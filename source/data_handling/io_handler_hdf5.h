...
// *
// *   choose between big-endian and little-endian format, and it allows the
// user  *
...
// *
// *   explicit reset can also be done.  Navigating using "cd" resets the
// checksum.*
...
public:
  ...
  void write(const std::string& objname, const bool& output);
  ...
  // Vector<T> support - Vector is KBfit's custom vector class
  template <typename T>
  void write(const std::string& objname, const Vector<T>& output) {
    write(objname, output.c_vector());
  }
  ...
  void read(const std::string& objname, bool& input);

  // Vector<T> support - Vector is KBfit's custom vector class
  template <typename T>
  void read(const std::string& objname, Vector<T>& input) {
    std::vector<T> temp;
    read(objname, temp);
    input = Vector<T>(temp);
  }
...
private:
  ...
  void check_for_hid_failure2(hid_t id, const std::string& mesg1 = "",
                              const std::string& mesg2 = "") const;
  void check_for_herr_failure(herr_t status,
  ...
  std::vector<std::string> tokenize(const std::string& str,
                                    ...
                                    bool allowdot = true,
                                    bool allowdotdot = false) const;
  void check_token(const std::string& token, bool allowdot = true,
                   bool allowdotdot = false) const;
  void check_path(const std::string& str, bool allowdot = true,
                  bool allowdotdot = false) const;
  std::string
  form_absolute_path(const std::vector<std::string>& path_links) const;
  ...
  void collect_data_names(hid_t loc_id, const std::string& path,
                          const char* name,
                          std::set<std::string>& collected_names) const;
  void collect_dir_names(hid_t loc_id, const std::string& path,
                         const char* name,
  ...
  void peek_strings(const std::string& filename,
  ...
  template <typename T>
  void read_atomic(const std::string& objname, T& input, hid_t dtype_id,
                   hid_t mem_type_id);
  template <typename T>
  ...
  void read_complex_vector(const std::string& objname,
                           std::vector<std::complex<T>>& output, hid_t dtype_id,
                           hid_t mem_type_id);

  template <typename T>
  void read_array(const std::string& objname, Array<T>& output, hid_t dtype_id,
                  hid_t mem_type_id);
...
template <typename T>
void IOHDF5Handler::write_atomics(const std::string& objname, const T* output,
                                  size_t nT, hid_t dtype_id,
                                  hid_t mem_type_id) {
  if (!openflag) {
  ...
  hid_t dataset_id =
      H5Dcreate2(*cwdptr, objname.c_str(), dtype_id, dataspace_id, H5P_DEFAULT,
                 H5P_DEFAULT, H5P_DEFAULT);
  check_for_hid_failure(dataset_id, "Could not create object name for writing");
  herr_t status =
      H5Dwrite(dataset_id, mem_type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, output);
  ...
  if (checksum_on)
    checksum = m_bytehandler.get_checksum(
        checksum, reinterpret_cast<const char*>(output), nT * sizeof(T));
}

template <typename T>
void IOHDF5Handler::write_complex_values(const std::string& objname,
                                         const std::complex<T>* output,
                                         size_t n, hid_t dtype_id,
                                         hid_t mem_type_id) {
  if (!openflag) {
    check_for_failure(IO_ERR_ACCESS, "Attempt to write when no open file");
  }
  ...
  if (checksum_on)
    checksum = m_bytehandler.get_checksum(
        checksum, reinterpret_cast<const char*>(output), 2 * n * sizeof(T));
}

template <typename T>
void IOHDF5Handler::write_array(const std::string& objname,
                                const Array<T>& output, hid_t dtype_id,
                                hid_t mem_type_id) {
  if (!openflag) {
    check_for_failure(IO_ERR_ACCESS, "Attempt to write when no open file");
  }
  ...
  if (checksum_on)
    checksum = m_bytehandler.get_checksum(
        checksum, reinterpret_cast<const char*>(output.m_store.data()),
        output.size() * sizeof(T));
}

template <typename T>
void IOHDF5Handler::write_complex_array(const std::string& objname,
                                        const Array<T>& output, hid_t dtype_id,
                                        hid_t mem_type_id) {
  if (!openflag) {
    check_for_failure(IO_ERR_ACCESS, "Attempt to write when no open file");
  ...
  for (uint k = 1; k < rank; ++k)
    dims[k] = output.size(k - 1);
  hid_t dataspace_id = H5Screate_simple(rank, dims, NULL);
  std::string obj(tidyString(objname));
  hid_t* cwdptr = (obj[0] == '/') ? (&fid) : (&currid);
  hid_t dataset_id =
...
void IOHDF5Handler::read_atomic(const std::string& objname, T& input,
  ...
  if (!H5Tequal(dtype, dtype_id))
    check_for_failure(IO_ERR_OTHER, "Datatype mismatch during read");
  hid_t dataspace_id = H5Dget_space(dataset_id);
  herr_t status =
      H5Dread(dataset_id, mem_type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, &input);
  ...
  if (checksum_on)
    checksum = m_bytehandler.get_checksum(
        checksum, reinterpret_cast<const char*>(&input), sizeof(T));
}

template <typename T>
void IOHDF5Handler::read_atomics(const std::string& objname,
                                 std::vector<T>& input, hid_t dtype_id,
                                 hid_t mem_type_id) {
  if (!openflag) {
    check_for_failure(IO_ERR_ACCESS, "Attempt to read when no open file");
  }
  ...
  if (rank != 1)
    check_for_failure(IO_ERR_OTHER, "Datatype rank mismatch during read");
  herr_t status = H5Sget_simple_extent_dims(dataspace_id, &dsize, NULL);
  input.resize(dsize);
  status = H5Dread(dataset_id, mem_type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   input.data());
  herr_t status1 = H5Sclose(dataspace_id);
  status1 = H5Dclose(dataset_id);
  check_for_herr_failure(status, "Could not read data");
  ...
  if (checksum_on)
    checksum = m_bytehandler.get_checksum(
        checksum, reinterpret_cast<const char*>(input.data()),
        input.size() * sizeof(T));
}

template <typename T>
void IOHDF5Handler::read_complex(const std::string& objname,
                                 std::complex<T>& input, hid_t dtype_id,
                                 hid_t mem_type_id) {
  if (!openflag) {
    check_for_failure(IO_ERR_ACCESS, "Attempt to read when no open file");
  }
  ...
  if (checksum_on)
    checksum = m_bytehandler.get_checksum(
        checksum, reinterpret_cast<const char*>(&input), 2 * sizeof(T));
}
...
void IOHDF5Handler::read_array(const std::string& objname, Array<T>& input,
  ...
  if (checksum_on)
    checksum = m_bytehandler.get_checksum(
        checksum, reinterpret_cast<const char*>(input.m_store.data()),
        input.size() * sizeof(T));
}

template <typename T>
void IOHDF5Handler::read_complex_array(const std::string& objname,
                                       Array<T>& input, hid_t dtype_id,
                                       hid_t mem_type_id) {
  if (!openflag) {
    check_for_failure(IO_ERR_ACCESS, "Attempt to read when no open file");
  }
