...
MCBinsInfo::MCBinsInfo(XMLHandler& xml_in) {
  ...
  try {
    ...
    if (xmli.queryTag("MCBinsInfo")) {
      ...
      XMLHandler xmlenn;
      xmlen.getInput(xmlenn);
      uint nmeas = 0;
      xmlr.getOptionalUInt("NumberOfMeasurements", nmeas);
      m_ensemble = new MCEnsembleInfo(xmlenn, nmeas);
    ...
    throw(std::invalid_argument(string("Invalid MCBinsInfo construction: ") +
                                xp.what()));
  }
...
void MCBinsInfo::setRebin(int rebin) {
  if ((rebin > 0) && (rebin <= (1 + int(m_nmeasures / 24)))) {
    m_rebin = rebin;
    reset_nbins();
  } else {
...
void MCBinsInfo::addOmissions(set<int> indices) {
  for (set<int>::const_iterator it = indices.begin(); it != indices.end();
       it++) {
    if ((*it >= 0) && (*it < int(m_nmeasures)))
      m_omit.insert(*it);
  }
  reset_nbins();
}
...
bool MCBinsInfo::isConsistentWith(const MCBinsInfo& rhs) const {
  if (*m_ensemble != *(rhs.m_ensemble))
    return false;
  if (m_omit != rhs.m_omit)
    return false;
  if ((rhs.m_rebin % m_rebin) != 0)
    return false;
  return true;
}
...
void MCBinsInfo::output(XMLHandler& xmlout) const {
  ...
  if ((m_rebin > 1) || (!m_omit.empty())) {
    ...
    if (m_rebin > 1)
      xmltw.put_child("Rebin", make_string(m_rebin));
    if (!(m_omit.empty())) {
      string omstr;
      for (set<unsigned int>::const_iterator it = m_omit.begin();
           it != m_omit.end(); it++)
