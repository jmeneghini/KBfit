...
// *
// *  object is like a C++ map, but instead of dealing with memory, it deals
// with  *
// *  data on a disk.  HDF5 files can be very large.  An IOHDF5Map is used to
// only *
// *  access a small part of the file.  You MUST specify a root path (group),
// then *
// *  only data in that group will be accessed.  In addition to the keys and
// data, *
...
// * *
// *   (1) a write(ioh, const V&) must be defined (ioh is an IOHDF5Handler
// object) *
...
// * *
// *  An ID string is used to identify an IOHDF5MapHDF5 file. You should choose
// a  *
...
template <typename K, typename V> class IOHDF5Map : public IOMapBase<K, V> {
...
public:
  ...
  virtual void openReadOnly(const std::string& filename_and_rootpath,
                            const std::string& filetype_id, std::string& header,
  ...
  virtual bool get_maybe(const K& key,
                         V& val); // returns false is fails, true otherwise

  virtual bool exist(const K& key) const;

  virtual void
  flush(); // puts file in finalized state so no data loss if abort occurs
  ...
  virtual bool
  keepKeys(const std::set<K>&
               keys_to_keep); // keep only those keys in "keys_to_keep"
                              // return true if all keys in "keys_to_keep" are
...
char IOHDF5Map<K, V>::get_filename_and_path(
    const std::string& filename_and_rootpath, std::string& filename,
    std::string& rootpath) {
  filename.clear();
  rootpath.clear();
  size_t nlbrack = std::count(filename_and_rootpath.begin(),
                              filename_and_rootpath.end(), '[');
  size_t nrbrack = std::count(filename_and_rootpath.begin(),
                              filename_and_rootpath.end(), ']');
  ...
  rootpath = tidyString(
      filename_and_rootpath.substr(rstart + 1, rfinish - rstart - 1));
  if (filename.empty())
    return 'N';
  if (rootpath.empty())
    return 'F';
...
void IOHDF5Map<K, V>::openReadOnly(const std::string& filename_and_rootpath,
  ...
  check_for_failure(ioh.isOpen(),
                    "Please do not open an already open IOHDF5Map!");
...
void IOHDF5Map<K, V>::openNew(const std::string& filename_and_rootpath,
  ...
  check_for_failure(ioh.isOpen(),
                    "Please do not open an already open IOHDF5Map!");
  OpenMode mode = (fail_if_exists) ? IOHDF5Handler::ReadWriteFailIfExists
                                   : IOHDF5Handler::ReadWriteEraseIfExists;
...
template <typename K, typename V>
void IOHDF5Map<K, V>::initialize(const std::string& filename_and_rootpath,
                                 OpenMode mode, const std::string& filetype_id,
                                 char endianness, bool turn_on_checksum,
  ...
  check_for_failure(
      flag == 'N',
      "Problem initializing IOHDF5Map: bad file name or root path");
  ...
  if (flag == 'F') {
    ...
    for (std::set<std::string>::iterator it = rootpaths.begin();
         it != rootpaths.end(); ++it) {
  ...
  if (ioh.isNewFile()) { // this is a new file
    if (verbose1)
      std::cout << "IOHDF5Map: opening new file " << filename << std::endl;
    ...
    if (verbose1)
      std::cout << "IOHDF5Map: opening already existing file " << filename
                << std::endl;
...
template <typename K, typename V> void IOHDF5Map<K, V>::close() {
  if (!ioh.isOpen())
    return;
  if (verbose1)
    std::cout << "IOHDF5Map: closed file " << ioh.getFileName() << std::endl;
  ioh.close();
}

template <typename K, typename V> void IOHDF5Map<K, V>::flush() {
  if (!ioh.isOpen())
    return;
  if (verbose1)
    std::cout << "IOHDF5Map: flushed file " << ioh.getFileName() << std::endl;
}

template <typename K, typename V> void IOHDF5Map<K, V>::setHighVerbosity() {
  verbose1 = verbose2 = true;
}

template <typename K, typename V> void IOHDF5Map<K, V>::setMediumVerbosity() {
  verbose1 = true;
  verbose2 = false;
}
...
template <typename K, typename V> std::string IOHDF5Map<K, V>::getHeader() {
...
template <typename K, typename V> uint IOHDF5Map<K, V>::get_size() const {
...
template <typename K, typename V> uint IOHDF5Map<K, V>::size() const {
  return get_size();
}
...
void IOHDF5Map<K, V>::put(const K& key, const V& val) {
  ...
  if (flag) {
    ...
    if (verbose2)
      std::cout << "IOHDF5Map::put of new record" << std::endl;
  }
  if (checksums_in_file)
    ioh.turnOnChecksum();
...
void IOHDF5Map<K, V>::get(const K& key, V& val) {
  ...
  if (verbose2)
    std::cout << "IOHDF5Map::get " << std::endl;
  std::string keyname(key.serialize());
...
bool IOHDF5Map<K, V>::get_maybe(const K& key, V& val) {
  if (!exist(key))
    return false;
  if (verbose2)
    std::cout << "IOHDF5Map::get_maybe " << std::endl;
  std::string keyname(key.serialize());
  ioh.read(std::string("Values/") + keyname, val);
  if (use_checksums) {
    ByteHandler::n_uint32_t checksumA = ioh.getChecksum();
...
void IOHDF5Map<K, V>::getKeys(std::set<K>& keys) const {
  ...
  ioh.cd("..");
  for (std::set<std::string>::iterator it = keynames.begin();
       it != keynames.end(); ++it)
    keys.insert(K(*it));
}
...
std::set<std::string> IOHDF5Map<K, V>::getAvailableRootPaths() const {
  check_for_failure(!ioh.isOpen(),
                    "IOHDF5Map not open: cannot get available root paths");
  std::set<std::string> alldirs(ioh.getAllDirNames());
  std::set<std::string> rootpaths;
  for (std::set<std::string>::const_iterator it = alldirs.begin();
       it != alldirs.end(); ++it) {
