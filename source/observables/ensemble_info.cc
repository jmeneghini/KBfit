...
// This constructor does not allow "indep"
...
// This constructor does allow "indep"

MCEnsembleInfo::MCEnsembleInfo(XMLHandler& xml_in, uint nsamplings) {
  XMLHandler xmlr(xml_in);
  xmlread(xmlr, "MCEnsembleInfo", m_id, "MCEnsembleInfo");
  if (m_id != "indep") {
    initialize();
...
// special ensemble-independent constructor

MCEnsembleInfo::MCEnsembleInfo(uint nsamplings) : m_id("indep") {
  n_streams = 1;
  n_meas = nsamplings;
  n_x = n_y = n_z = 0;
  n_t = 0;
...
bool MCEnsembleInfo::parse(const string& idstr) {
  ...
  try {
    ...
    extract_from_string(idstr.substr(pos[1] + 1, pos[2] - pos[1] - 1),
                        n_streams);
...
MCEnsembleInfo::MCEnsembleInfo(const std::string& id, uint num_meas,
                               uint num_streams, uint nx, uint ny, uint nz,
                               uint nt)
    : m_id(tidyString(id)), n_meas(num_meas), n_streams(num_streams), n_x(nx),
      n_y(ny), n_z(nz), n_t(nt) {
  ostringstream oss;
  oss << "|" << n_meas << "|" << n_streams << "|" << n_x << "|" << n_y << "|"
      << n_z << "|" << n_t;
  m_id += oss.str();
}
