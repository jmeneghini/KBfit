...
std::unique_ptr<KtildeMatrixCalculator> KtildeMatrixCalculator::clone() const {
  ...
  auto cloned =
      std::unique_ptr<KtildeMatrixCalculator>(new KtildeMatrixCalculator());
  ...
  for (const auto& pair : this->m_fit) {
    ...
    if (original_fitform != nullptr) {
      ...
      if (const Polynomial* poly =
              dynamic_cast<const Polynomial*>(original_fitform)) {
        cloned_fitform = new Polynomial(*poly);
      } else if (const SumOfPoles* sumpoles =
                     dynamic_cast<const SumOfPoles*>(original_fitform)) {
        cloned_fitform = new SumOfPoles(*sumpoles);
      } else if (const SumOfPolesPlusPolynomial* sumpolespoly =
                     dynamic_cast<const SumOfPolesPlusPolynomial*>(
                         original_fitform)) {
        cloned_fitform = new SumOfPolesPlusPolynomial(*sumpolespoly);
      } else if (const Expression* expr =
                     dynamic_cast<const Expression*>(original_fitform)) {
...
std::unique_ptr<KtildeInverseCalculator>
KtildeInverseCalculator::clone() const {
  // Create new instance using default constructor
  auto cloned =
      std::unique_ptr<KtildeInverseCalculator>(new KtildeInverseCalculator());
