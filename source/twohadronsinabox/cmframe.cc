...
void EcmTransform::initialize() {
  ...
  double minEcmsq =
      abs(m1_over_mref - m2_over_mref) * (m1_over_mref + m2_over_mref);
...
double EcmTransform::getUsquared(double Elab_over_mref) const {
  ...
  double mm2 = m2_over_mref * m2_over_mref;
  double qcmsq =
      0.25 * Ecmsq - 0.5 * (mm1 + mm2) + 0.25 * sqr(mm1 - mm2) / Ecmsq;
...
void EcmTransform::getGammaSvecUsq(double Elab_over_mref, double& gam,
                                   vector<double>& svec, double& usq) const {
  assertlabmin(Elab_over_mref);
  const double invtwopisquared = 0.025330295910584442860;
  double Ecmsq = Elab_over_mref * Elab_over_mref - Psq_over_mrefsq;
  double mm1 = m1_over_mref * m1_over_mref;
...
void EcmTransform::getGammaSvecUsqFromEcm(double Ecm_over_mref, double& gam,
                                          vector<double>& svec,
                                          double& usq) const {
  assertcmmin(Ecm_over_mref);
  const double invtwopisquared = 0.025330295910584442860;
...
void EcmTransform::getGammaSvecUsqFromQcmsq(double Qcmsq_over_mrefsq,
                                            double& gam, vector<double>& svec,
                                            double& usq) const {
  assertqcmmin(Qcmsq_over_mrefsq);
  const double invtwopisquared = 0.025330295910584442860;
  double mm1 = m1_over_mref * m1_over_mref;
  double mm2 = m2_over_mref * m2_over_mref;
  double Ecm = sqrt(Qcmsq_over_mrefsq + mm1) + sqrt(Qcmsq_over_mrefsq + mm2);
...
list<double> EcmTransform::getAllFreeTwoParticleEnergiesInElab(
  ...
  for (int nx = -nmax; nx <= nmax; nx++) {
    ...
    for (int ny = -nmax; ny <= nmax; ny++) {
      int n1xysq = n1xsq + ny * ny;
      int n2xysq = dy - ny;
      n2xysq *= n2xysq;
      n2xysq += n2xsq;
      for (int nz = -nmax; nz <= nmax; nz++) {
        zp.first = n1xysq + nz * nz;
        int n2zz = dz - nz;
        zp.second = n2zz * n2zz + n2xysq;
        if ((equalmasses) && (zp.first > zp.second)) {
          unsigned int tmp = zp.first;
          zp.first = zp.second;
          zp.second = tmp;
        }
        it = Efreemap.find(zp);
        if (it == Efreemap.end()) { // not yet in map
          Epole = sqrt(pfac * double(zp.first) + mm1) +
                  sqrt(pfac * double(zp.second) + mm2);
