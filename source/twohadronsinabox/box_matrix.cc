...
BoxMatrix::BoxMatrix(const EcmTransform& incm, WZetaRGLCalculator& wzetacalc,
  ...
  stringstream s;
  s << total_spin_times_two;
  block_id += string("_") + s.str() + string("_") + lgirrep;
  std::map<std::string, EvaluatorPtr>::iterator eit =
      evaluatorMap.find(block_id);
  std::map<std::string, BlockSetupPtr>::iterator iit =
      blockSetupMap.find(block_id);
...
uint BoxMatrix::getElementIndex(const BoxMatrixQuantumNumbers& bqn) const {
  ...
  if (it == m_indexmap.end())
    throw(std::runtime_error(string("Quantum numbers not found ") +
                             bqn.output()));
  return it->second;
}

int BoxMatrix::getElementIndexNegIfFail(
    const BoxMatrixQuantumNumbers& bqn) const {
  std::map<BoxMatrixQuantumNumbers, uint>::const_iterator it =
      m_indexmap.find(bqn);
  if (it == m_indexmap.end())
    return -1;
...
void BoxMatrix::setElementsFromElab(double Elab_over_mref) {
  ...
  double bf = m_cmt.getQcmsqOverMrefsq(Elab_over_mref);
...
void BoxMatrix::setElementsFromEcm(double Ecm_over_mref) {
  ...
  double bf = m_cmt.getQcmsqOverMrefsqFromEcm(Ecm_over_mref);
  m_wzetas.reset_if_diff(svec, gam, usq);
  (*m_evalptr)(m_wzetas, m_nelem, m_results);
  barrier_rescale(bf);
}
...
void BoxMatrix::setElementsFromQcmsq(double qcmsq_over_mrefsq) {
  ...
  m_wzetas.reset_if_diff(svec, gam, usq);
  (*m_evalptr)(m_wzetas, m_nelem, m_results);
  barrier_rescale(qcmsq_over_mrefsq);
}

//  multiplies m_results by  bf^(rowL+colL+1)

void BoxMatrix::barrier_rescale(double bf) {
  vector<cmplx> u_powers(std::max(2 * m_lmax + 1, 2u));
  u_powers[1] = cmplx(bf, 0.0);
  if (bf >= 0.0)
    u_powers[0] = cmplx(sqrt(bf), 0.0);
  else
    u_powers[0] = cmplx(0.0, sqrt(-bf));
...
void BoxMatrix::getElementsFromElab(double Elab_over_mref,
                                    std::vector<cmplx>& results) {
  setElementsFromElab(Elab_over_mref);
  results = m_results;
}

void BoxMatrix::getElementsFromEcm(double Ecm_over_mref,
                                   std::vector<cmplx>& results) {
  setElementsFromEcm(Ecm_over_mref);
  results = m_results;
}

void BoxMatrix::getElementsFromQcmsq(double qcmsq_over_mrefsq,
                                     std::vector<cmplx>& results) {
