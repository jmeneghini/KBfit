...
// *   This class computes                                               *
// *                                                                     *
// *         det(1+Stilde*CB)   or       det(Stildeinverse+CB)           *
// *                                                                     *
// *         det(1-Ktilde*B)    or       det(Ktildeinverse-B)            *
// *                                                                     *
...
// *                                                                     *
// *   Note: these definitions of Ktildeinv and the box matrix differ    *
// *   from Eq. (19) and Eq. (21) in Nucl. Phys. B924, 477 (2017).       *
// *   The original definitions caused a dependence on the lattice size  *
// *   of Ktildeinv which is important to avoid.                         *
// *                                                                     *
// *   The Cayley transformed matrices are defined by                    *
// *                                                                     *
// *        CB = (1 + i*B) * (1 - i*B)^(-1)                              *
// *                                                                     *
// *        Stilde = (1 + i*Ktilde) * (1-i*Ktilde)^(-1)                  *
// *                                                                     *
// *               = -(1 - i*Ktildeinverse) * (1 + i*Ktildeinverse)^(-1) *
// *                                                                     *
...
//    enum QuantMat { StildeCB, StildeinvCB, KtildeB, KtildeinvB };
...
public:
  ...
  void getCTBoxMatrixFromElab(double Elab_over_mref, CMatrix& CB);

  void getCTBoxMatrixFromEcm(double Ecm_over_mref, CMatrix& CB);

  void getStildeFromElab(double Elab_over_mref, CMatrix& Stilde);
  ...
  std::vector<cmplx> getQCEigenvaluesFromEcm(double Ecm_over_mref,
                                             QuantCondType qctype);
...
private:
  ...
  const std::unordered_map<std::string_view, QuantCondType> m_qctype_map = {
      {"stildecb", StildeCB},
      {"stildeinvcb", StildeinvCB},
      {"ktildeb", KtildeB},
      {"ktildeinvb", KtildeinvB}};
  ...
  void assign_ct_matrices(double E_over_mref, bool Elab, CMatrix& CB,
                          CMatrix& Stilde);
  ...
  void output_ct_matrices(double E_over_mref, bool Elab, std::ostream& fout);

  std::vector<cmplx> get_qc_eigenvalues(double E_over_mref, bool Elab,
                                        QuantCondType qctype);
