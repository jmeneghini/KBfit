...
YLMpoly::YLMpoly(const YLMpoly& in)
    : m_lval(in.m_lval), m_mval(in.m_mval), m_cf(in.m_cf),
      m_x3flag(in.m_x3flag), m_pdeg(in.m_pdeg), m_pcf(in.m_pcf),
      m_buf(in.m_buf) {}
...
YLMpoly::YLMpoly(int lvalue, int mvalue) {
  ...
  if ((mvalue < 0) || (mvalue > lvalue)) {
    throw(
        std::invalid_argument("Unsupported or invalid value of M in YLMpoly"));
  }
  m_lval = lvalue;
  m_mval = mvalue;
...
cmplx YLMpoly::value(double x1, double x2, double x3) const {
  if (m_lval == 0)
    return cmplx(0.28209479177387814348, 0.0);
...
cmplx YLMpoly::azimuthal(double x1, double x2) const {
  if (m_mval == 0)
    return cmplx(m_cf, 0.0);
  else if (m_mval == 1)
    return cmplx(m_cf * x1, m_cf * x2);
...
double YLMpoly::azimuthal_realpart(double x1, double x2) const {
  ...
  case 6: {
    double xx1 = x1 * x1, xx2 = x2 * x2;
    return m_cf * (x1 - 1.0 * x2) * (x1 + x2) * (xx1 - 4.0 * x1 * x2 + xx2) *
           (xx1 + 4.0 * x1 * x2 + xx2);
  }
  default: {
    cmplx xy(x1, x2), res(1.0, 0.0);
...
bool zetaRGL_RestFrame_IsZero(int l, int m) {
  if ((l % 2) == 1)
    return true; // zero if l is odd
  if ((m % 4) != 0)
    return true; // zero if m not multiple of
  if (l == 2)
    return true; // zero if l is 2
  return false;
}
...
inline void do_sum(K& res, T& summand, int nnmin, double abstol, double reltol,
  ...
  for (int nn = nnmin + 1; nn <= nnmax; nn++) {
    ...
    if ((addmag < abstol) || (addmag < reltol * resmag))
      return;
  }
...
template <typename T, typename K>
inline void do_simple_sum(K& res, T& summand, int nmax, int offset1 = 0,
                          int offset2 = 0, int offset3 = 0) {
  res = 0;
...
template <typename T> class GQvec {
...
template <typename T> double abs(const GQvec<T>& in) { return in.abs(); }

void assign_gq_abscissae_weights(GQvec<double>& abscissae,
                                 GQvec<double>& weights) {
  abscissae[0] = 0.001368069075259218227509;
  weights[0] = 0.003509305004735048294500;
  abscissae[1] = 0.007194244227365832299912;
  weights[1] = 0.008137197365452835311006;
  abscissae[2] = 0.017618872206246784613094;
  weights[2] = 0.012696032654631029765878;
  abscissae[3] = 0.032546962031130155414540;
  weights[3] = 0.017136931456510716555404;
  abscissae[4] = 0.051839422116973938017346;
  weights[4] = 0.021417949011113340279872;
  abscissae[5] = 0.075316193133715014933153;
  weights[5] = 0.025499029631188088088508;
  abscissae[6] = 0.102758102016028796518451;
  weights[6] = 0.029342046739267773554389;
  abscissae[7] = 0.133908940629855159806287;
  weights[7] = 0.032911111388180923418612;
  abscissae[8] = 0.168477866534892399512442;
  weights[8] = 0.036172897054424253112176;
  abscissae[9] = 0.206142121379618835479627;
  weights[9] = 0.039096947893535153236086;
  abscissae[10] = 0.246550045533885304988126;
  weights[10] = 0.041655962113473377611097;
  abscissae[11] = 0.289324361934682327317940;
  weights[11] = 0.043826046502201905571388;
  abscissae[12] = 0.334065698858936175110042;
  weights[12] = 0.045586939347881942356434;
  abscissae[13] = 0.380356318873931462727698;
  weights[13] = 0.046922199540402282819590;
  abscissae[14] = 0.427764019208601753257407;
  weights[14] = 0.047819360039637429709541;
  abscissae[15] = 0.475846167156130841882594;
  weights[15] = 0.048270044257363900283382;
  abscissae[16] = 0.524153832843869158117406;
  weights[16] = 0.048270044257363900283382;
  abscissae[17] = 0.572235980791398246742593;
  weights[17] = 0.047819360039637429709541;
  abscissae[18] = 0.619643681126068537272302;
  weights[18] = 0.046922199540402282819590;
  abscissae[19] = 0.665934301141063824889958;
  weights[19] = 0.045586939347881942356434;
  abscissae[20] = 0.710675638065317672682060;
  weights[20] = 0.043826046502201905571388;
  abscissae[21] = 0.753449954466114695011874;
  weights[21] = 0.041655962113473377611097;
  abscissae[22] = 0.793857878620381164520373;
  weights[22] = 0.039096947893535153236086;
  abscissae[23] = 0.831522133465107600487558;
  weights[23] = 0.036172897054424253112176;
  abscissae[24] = 0.866091059370144840193713;
  weights[24] = 0.032911111388180923418612;
  abscissae[25] = 0.897241897983971203481549;
  weights[25] = 0.029342046739267773554389;
  abscissae[26] = 0.924683806866284985066847;
  weights[26] = 0.025499029631188088088508;
  abscissae[27] = 0.948160577883026061982654;
  weights[27] = 0.021417949011113340279872;
  abscissae[28] = 0.967453037968869844585460;
  weights[28] = 0.017136931456510716555404;
  abscissae[29] = 0.982381127793753215386906;
  weights[29] = 0.012696032654631029765878;
  abscissae[30] = 0.992805755772634167700088;
  weights[30] = 0.008137197365452835311006;
  abscissae[31] = 0.998631930924740781772491;
  weights[31] = 0.003509305004735048294500;
}
/*
void assign_gq_abscissae_weights(GQvec<double>& abscissae,
                                 GQvec<double>& weights)
 abscissae[ 0]=  0.000614496373786940699729;  weights[  0]=
0.001576673026155008503200; abscissae[ 1]=  0.003234913866824621226036; weights[
1]=  0.003663776950666149277350; abscissae[ 2]=  0.007937708138586571127708;
weights[  2]=  0.005738617289622212345074; abscissae[ 3]=
0.014704203726876374769294;  weights[  3]=  0.007789657861441853379128;
 abscissae[ 4]=  0.023506148419784569638520;  weights[  4]=
0.009808080228715613437210; abscissae[ 5]=  0.034306654646722833442913; weights[
5]=  0.011785380419568279119624; abscissae[ 6]=  0.047060431642215163588963;
weights[  6]=  0.013713254854188918531323; abscissae[ 7]=
0.061713989862876057047153;  weights[  7]=  0.015583613916411056588286;
 abscissae[ 8]=  0.078205869187803234644455;  weights[  8]=
0.017388611282389094163839; abscissae[ 9]=  0.096466897985278686458723; weights[
9]=  0.019120675532914933319873; abscissae[10]=  0.116420483742129830373072;
weights[ 10]=  0.020772541471732020449686; abscissae[11]=
0.137982934538092672662759;  weights[ 11]=  0.022337280428347154754836;
 abscissae[12]=  0.161063810183668047394074;  weights[ 12]=
0.023808329246245221401385; abscissae[13]=  0.185566301611743188002418; weights[
13]=  0.025179517776927238125942; abscissae[14]=  0.211387636958013648091095;
weights[ 14]=  0.026445094742596834057610; abscissae[15]=
0.238419512638883483160887;  weights[ 15]=  0.027599751849992081410365;
 abscissae[16]=  0.266548547624520797727536;  weights[ 16]=
0.028638646050201607850234; abscissae[17]=  0.295656759004641635041887; weights[
17]=  0.029557419849197817872816; abscissae[18]=  0.325622056853919630920091;
weights[ 18]=  0.030352219582946940026474; abscissae[19]=
0.356318756322272211632057;  weights[ 19]=  0.031019711579946331952098;
 abscissae[20]=  0.387618104802655469387567;  weights[ 20]=
0.031557096143127012828563; abscissae[21]=  0.419388821965554140971781; weights[
21]=  0.031962119292324093311953; abscissae[22]=  0.451497650395268650534973;
weights[ 22]=  0.032233082217975041103252; abscissae[23]=
0.483809914518565318983339;  weights[ 23]=  0.032368848406341961251512;
 abscissae[24]=  0.516190085481434681016661;  weights[ 24]=
0.032368848406341961251512; abscissae[25]=  0.548502349604731349465027; weights[
25]=  0.032233082217975041103252; abscissae[26]=  0.580611178034445859028219;
weights[ 26]=  0.031962119292324093311953; abscissae[27]=
0.612381895197344530612433;  weights[ 27]=  0.031557096143127012828563;
 abscissae[28]=  0.643681243677727788367943;  weights[ 28]=
0.031019711579946331952098; abscissae[29]=  0.674377943146080369079909; weights[
29]=  0.030352219582946940026474; abscissae[30]=  0.704343240995358364958113;
weights[ 30]=  0.029557419849197817872816; abscissae[31]=
0.733451452375479202272464;  weights[ 31]=  0.028638646050201607850234;
 abscissae[32]=  0.761580487361116516839113;  weights[ 32]=
0.027599751849992081410365; abscissae[33]=  0.788612363041986351908905; weights[
33]=  0.026445094742596834057610; abscissae[34]=  0.814433698388256811997582;
weights[ 34]=  0.025179517776927238125942; abscissae[35]=
0.838936189816331952605926;  weights[ 35]=  0.023808329246245221401385;
 abscissae[36]=  0.862017065461907327337241;  weights[ 36]=
0.022337280428347154754836; abscissae[37]=  0.883579516257870169626928; weights[
37]=  0.020772541471732020449686; abscissae[38]=  0.903533102014721313541277;
weights[ 38]=  0.019120675532914933319873; abscissae[39]=
0.921794130812196765355545;  weights[ 39]=  0.017388611282389094163839;
 abscissae[40]=  0.938286010137123942952847;  weights[ 40]=
0.015583613916411056588286; abscissae[41]=  0.952939568357784836411037; weights[
41]=  0.013713254854188918531323; abscissae[42]=  0.965693345353277166557087;
weights[ 42]=  0.011785380419568279119624; abscissae[43]=
0.976493851580215430361480;  weights[ 43]=  0.009808080228715613437210;
 abscissae[44]=  0.985295796273123625230706;  weights[ 44]=
0.007789657861441853379128; abscissae[45]=  0.992062291861413428872292; weights[
45]=  0.005738617289622212345074; abscissae[46]=  0.996765086133175378773964;
weights[ 46]=  0.003663776950666149277350; abscissae[47]=
0.999385503626213059300271;  weights[ 47]=  0.001576673026155008503200;
}
*/
// ***********************************************************
...
ZetaSummand1_re::ZetaSummand1_re(int lval, int mval, double uu)
    : ylm(lval, mval) {
  usq = uu;
}
...
ZetaSummand1_im::ZetaSummand1_im(int lval, int mval, double uu)
    : ylm(lval, mval) {
  usq = uu;
}
...
inline const GQvec<cmplx>& ZetaSummand2::operator()(int n1, int n2,
                                                    int n3) const {
...
inline const GQvec<cmplx>& ZetaSummand2s::operator()(int n1, int n2,
                                                     int n3) const {
...
inline const GQvec<double>& ZetaSummand2_re::operator()(int n1, int n2,
                                                        int n3) const {
  const double pipi = 9.86960440108935861883447;
  double w1 = n1;
  double w2 = n2;
...
inline const GQvec<double>& ZetaSummand2s_re::operator()(int n1, int n2,
                                                         int n3) const {
  const double pi = 3.14159265358979323846264;
  const double pipi = 9.86960440108935861883447;
  double wb = n1 * s1 + n2 * s2 + n3 * s3;
...
inline const GQvec<double>& ZetaSummand2_im::operator()(int n1, int n2,
                                                        int n3) const {
  const double pipi = 9.86960440108935861883447;
  double w1 = n1;
  double w2 = n2;
...
inline const GQvec<double>& ZetaSummand2s_im::operator()(int n1, int n2,
                                                         int n3) const {
  const double pi = 3.14159265358979323846264;
  const double pipi = 9.86960440108935861883447;
  double wb = n1 * s1 + n2 * s2 + n3 * s3;
...
cmplx zeta_noshift(int l, int m, double usq) {
  if (zetaRGL_RestFrame_IsZero(l, m))
  ...
  nnmin = 6;
  do_sum<ZetaSummand2, GQvec<cmplx>>(intgrand, summand2, nnmin, abstol, reltol,
                                     true);

  cmplx res2(0.0, 0.0);
  for (int k = 0; k < tvals.npoints(); k++) {
    res2 += wts[k] * intgrand[k] * exp(tvals[k] * usq) /
            pow(tvals[k], 1.5 + double(l));
  }
...
cmplx zeta_shifted(int l, int m, const vector<double>& s, double ss, double gam,
                   double usq) {
  const double pi = 3.14159265358979323846264;
  ...
  int nn = ceil(std::abs(s[1]));
  if (nn > nnmin)
    nnmin = nn;
  nn = ceil(std::abs(s[2]));
  if (nn > nnmin)
    nnmin = nn;
  nn = ceil(sqrt(std::abs(usq)));
  if (nn > nnmin)
    nnmin = nn;
  nnmin += 6;
  double abstol = 1e-18, reltol = 1e-12;
  ...
  GQvec<cmplx> intgrand;
  nnmin = 6;
  do_sum<ZetaSummand2s, GQvec<cmplx>>(intgrand, summand2, nnmin, abstol, reltol,
                                      true);
...
double high_zeta_noshift_re(int l, int m, double usq, double abstol,
                            double reltol) {
  const double pi = 3.14159265358979323846264;
  GQvec<double> tvals, wts;
  assign_gq_abscissae_weights(tvals, wts);
  ...
  int nnmin = 6;
  do_sum<ZetaSummand2_re, GQvec<double>>(intgrand, summand2, nnmin, abstol,
                                         reltol, true);
  double res2 = 0.0;
  for (int k = 0; k < tvals.npoints(); k++) {
    res2 += wts[k] * intgrand[k] * exp(tvals[k] * usq) /
...
double high_zeta_noshift_im(int l, int m, double usq, double abstol,
                            double reltol) {
  const double pi = 3.14159265358979323846264;
  GQvec<double> tvals, wts;
  assign_gq_abscissae_weights(tvals, wts);
  ...
  GQvec<double> intgrand;
  int nnmin = 6;
  do_sum<ZetaSummand2_im, GQvec<double>>(intgrand, summand2, nnmin, abstol,
                                         reltol, true);
  double res2 = 0.0;
  for (int k = 0; k < tvals.npoints(); k++) {
    res2 += wts[k] * intgrand[k] * exp(tvals[k] * usq) /
...
double high_zeta_shifted_re(int l, int m, const vector<double>& s, double ss,
                            double gam, double usq, double abstol,
                            double reltol) {
  const double pi = 3.14159265358979323846264;
  GQvec<double> tvals, wts;
  assign_gq_abscissae_weights(tvals, wts);
  ...
  GQvec<double> intgrand;
  int nnmin = 6;
  do_sum<ZetaSummand2s_re, GQvec<double>>(intgrand, summand2, nnmin, abstol,
                                          reltol, true);
  double res2 = 0.0;
  for (int k = 0; k < tvals.npoints(); k++) {
    res2 += wts[k] * intgrand[k] * exp(tvals[k] * usq) /
...
double high_zeta_shifted_im(int l, int m, const vector<double>& s, double ss,
  ...
  GQvec<double> intgrand;
  int nnmin = 6;
  do_sum<ZetaSummand2s_im, GQvec<double>>(intgrand, summand2, nnmin, abstol,
                                          reltol, true);
  double res2 = 0.0;
  for (int k = 0; k < tvals.npoints(); k++) {
    res2 += wts[k] * intgrand[k] * exp(tvals[k] * usq) /
...
double zeta_noshift_re(int l, int m, double usq) {
  ...
  nnmin += 6;
  double abstol = 1e-18, reltol = 1e-12;
  do_sum<ZetaSummand1_re, double>(res1, summand1, nnmin, abstol, reltol,
                                  oexclude);
...
double zeta_noshift_im(int l, int m, double usq) {
  ...
  nnmin += 6;
  double abstol = 1e-18, reltol = 1e-12;
  do_sum<ZetaSummand1_im, double>(res1, summand1, nnmin, abstol, reltol,
                                  oexclude);
...
double zetaRGL_re(int l, int m, const vector<double>& s, double gam,
                  double usq) {
  double ss = s[0] * s[0] + s[1] * s[1] + s[2] * s[2];
  if (ss < 1e-15) {
...
double zetaRGL_im(int l, int m, const vector<double>& s, double gam,
                  double usq) {
  double ss = s[0] * s[0] + s[1] * s[1] + s[2] * s[2];
  if (ss < 1e-15) {
...
WZetaRGLCalculator::WZetaRGLCalculator(const std::vector<double>& s, double gam,
                                       double usq)
...
void WZetaRGLCalculator::reset(const std::vector<double>& s, double gam,
                               double usq) {
...
void WZetaRGLCalculator::reset_if_diff(const std::vector<double>& s, double gam,
                                       double usq) {
