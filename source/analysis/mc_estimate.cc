...
double MCEstimate::getFullEstimate() const { return m_store[0]; }

double MCEstimate::getAverageEstimate() const { return m_store[1]; }

double MCEstimate::getSymmetricError() const { return m_store[2]; }

double MCEstimate::getLowerConfLimit() const {
  if (m_mode != Bootstrap)
    throw(std::invalid_argument(
        "Bootstrap mode required for getDownError in MCEstimate"));
  return m_store[3];
}
...
double MCEstimate::getMedian() const {
  if (m_mode != Bootstrap)
    throw(std::invalid_argument(
        "Bootstrap mode required for getMedian in MCEstimate"));
  return m_store[4];
}
...
double MCEstimate::getUpperConfLimit() const {
  if (m_mode != Bootstrap)
    throw(std::invalid_argument(
        "Bootstrap mode required for getUpError in MCEstimate"));
  return m_store[5];
}

bool MCEstimate::isJackknifeMode() const { return (m_mode == Jackknife); }

bool MCEstimate::isBootstrapMode() const { return (m_mode == Bootstrap); }
...
void MCEstimate::bootassign(double full, double avg, double error, double low,
                            double med, double upp) {
  m_store.resize(6);
  m_store[0] = full;
  m_store[1] = avg;
  m_store[2] = error;
...
string SimpleMCEstimate::str(unsigned int nerr_digits) const {
  ...
  if (exponent >= 0) {
    ...
    while (res[pos] != '(')
      pos++; // point to "("
    for (i = pos; i <= -exponent; i++)
      res = "0" + res; // pad with 0's on left
    pos = 0;
    while (res[pos] != '(')
      pos++; // point to "("
    pos += exponent;
