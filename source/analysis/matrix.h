...
// ***************************************************************
// *                                                             *
// *   The classes "Vector" and "Matrix" are defined in          *
// *   this file.  These are used by the data analysis           *
// *   routines.  "Vector" and "Matrix" are 1-dimensional and    *
// *   2-dimensional, respectively, versions of "Array".         *
// *   The class "LowerTriangularMatrix" is also defined,        *
// *   which is useful for Cholesky decompositions and only      *
// *   stores the nonzero elements in the lower triangle.        *
// *   These classes are templated.  Individual elements         *
// *   can be assigned and accessed with parentheses and         *
// *   square brackets, in the case of "Vector".  However,       *
// *   "put" and "get" methods are also available.               *
// *   "LowerTriangularMatrix" assumes a square matrix,          *
// *   and throws an exception when attempting to access an      *
// *   element in the upper right triangle.                      *
// *                                                             *
// *   All indices are ZERO offset.                              *
// *                                                             *
// *   Two other important classes defined here are              *
// *   "RealSymmetricMatrix" and "ComplexHermitianMatrix".       *
// *   Only the upper right triangle is stored to save memory.   *
// *   Assignment of elements for "ComplexHermitianMatrix"       *
// *   MUST be done using the "put" member, so that assignment   *
// *   to diagonal elements can be checked for a zero imaginary  *
// *   part.                                                     *
// *                                                             *
...
public:
  ...
  Vector(int size);
  Vector(uint size);
  Vector(int size, const T& initial_value);
  Vector(uint size, const T& initial_value);
  Vector(const Vector<T>& incoming);
  Vector(const std::vector<T>& incoming);
  ...
  Vector<T>& operator*=(const T& val);
  Vector<T>& operator/=(const T& val);
  Vector<T>& operator=(const Vector<T>& incoming);
  Vector<T>& operator+=(const Vector<T>& incoming);
  Vector<T>& operator-=(const Vector<T>& incoming);
  Vector<T>& operator*=(const Vector<T>& incoming);
  Vector<T>& operator/=(const Vector<T>& incoming);
  ...
  const T& operator()(int i) const;
  void put(int i, const T& inputvalue);
  ...
  const T& operator()(uint i) const;
  void put(uint i, const T& inputvalue);
...
template <typename T>
Vector<T>::Vector(int size) : m_store((size > 0) ? size : 0) {
  if (size < 1)
    throw(std::invalid_argument("Invalid Vector size"));
}

template <typename T> Vector<T>::Vector(uint size) : m_store(size) {}

template <typename T>
Vector<T>::Vector(int size, const T& initial_value)
    : m_store((size > 0) ? size : 0, initial_value) {
  if (size < 1)
    throw(std::invalid_argument("Invalid Vector size"));
}

template <typename T>
Vector<T>::Vector(uint size, const T& initial_value)
    : m_store(size, initial_value) {}

template <typename T>
Vector<T>::Vector(const Vector<T>& incoming) : m_store(incoming.m_store) {}

template <typename T>
Vector<T>::Vector(const std::vector<T>& incoming) : m_store(incoming) {}
...
template <typename T> inline Vector<T>& Vector<T>::operator=(const T& val) {
...
template <typename T>
inline Vector<T>& Vector<T>::operator=(const Vector<T>& incoming) {
  if (this == &incoming)
    return *this;
...
inline Vector<T>& Vector<T>::operator+=(const Vector<T>& incoming) {
#ifdef SAFETY_FLAG
  if (m_store.size() != incoming.m_store.size())
...
template <typename T> inline void Vector<T>::put(int i, const T& inputvalue) {
#ifdef SAFETY_FLAG
  if ((i < 0) || (i >= int(m_store.size())))
    throw(std::invalid_argument("Vector: index out of range"));
#endif
  m_store[i] = inputvalue;
}

template <typename T> inline const T& Vector<T>::get(int i) const {
#ifdef SAFETY_FLAG
  if ((i < 0) || (i >= int(m_store.size())))
    throw(std::invalid_argument("Vector: index out of range"));
#endif
...
template <typename T> inline T& Vector<T>::operator[](uint i) {
#ifdef SAFETY_FLAG
  if (i >= m_store.size())
    throw(std::invalid_argument("Vector: index out of range"));
#endif
...
template <typename T> inline const T& Vector<T>::operator[](uint i) const {
#ifdef SAFETY_FLAG
  if (i >= m_store.size())
    throw(std::invalid_argument("Vector: index out of range"));
#endif
...
template <typename T> inline void Vector<T>::put(uint i, const T& inputvalue) {
#ifdef SAFETY_FLAG
  if (i >= m_store.size())
    throw(std::invalid_argument("Vector: index out of range"));
#endif
...
template <typename T> inline const T& Vector<T>::get(uint i) const {
#ifdef SAFETY_FLAG
  if (i >= m_store.size())
    throw(std::invalid_argument("Vector: index out of range"));
#endif
...
template <typename T> Vector<T>& Vector<T>::resize() { return clear(); }
...
template <typename T> Vector<T>& Vector<T>::resize(uint size) {
  m_store.resize(size);
  return *this;
}

template <typename T> std::vector<T> Vector<T>::c_vector() const {
  return m_store;
}

template <typename T> std::vector<T>& Vector<T>::c_vector_ref() {
  return m_store;
}
...
public:
  ...
  void put(int i0, uint i1, const T& inputvalue) {
    put(i0, int(i1), inputvalue);
  }
  void put(uint i0, int i1, const T& inputvalue) {
    put(int(i0), i1, inputvalue);
  }
...
template <typename T> unsigned int Matrix<T>::set_sizes(int size0, int size1) {
  if ((size0 < 0) || (size1 < 0)) {
...
template <typename T> Matrix<T>::Matrix() : m_sizes(2) {
  m_sizes[0] = m_sizes[1] = 0;
}
...
template <typename T> Matrix<T>& Matrix<T>::clear() {
  m_store.clear();
  m_sizes[0] = m_sizes[1] = 0;
  return *this;
}

template <typename T> inline Matrix<T>& Matrix<T>::operator=(const T& val) {
  for (uint i = 0; i < m_store.size(); i++)
    m_store[i] = val;
  return *this;
}
...
#ifdef SAFETY_FLAG
  if ((i0 < 0) || (i0 >= int(m_sizes[0])))
    throw(std::invalid_argument("Matrix: 1st index out of range"));
  if ((i1 < 0) || (i1 >= int(m_sizes[1])))
    throw(std::invalid_argument("Matrix: 2nd index out of range"));
#endif
...
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument("Matrix: improper rank of Matrix indices"));
#endif
...
template <typename T> inline T& Matrix<T>::operator()(int i0, int i1) {
  return m_store[get_index(i0, i1)];
}

template <typename T> inline T& Matrix<T>::operator()(uint i0, uint i1) {
  return m_store[get_index(i0, i1)];
}
...
template <typename T> inline const T& Matrix<T>::get(int i0, int i1) const {
  return m_store[get_index(i0, i1)];
}

template <typename T> inline const T& Matrix<T>::get(uint i0, uint i1) const {
  return m_store[get_index(i0, i1)];
}
...
#ifdef SAFETY_FLAG
  if ((i < 0) || (i > 1))
    throw(std::invalid_argument("Matrix size index out of range"));
#endif
  return m_sizes[i];
}

template <typename T> inline uint Matrix<T>::size(uint i) const {
#ifdef SAFETY_FLAG
  if (i > 1)
    throw(std::invalid_argument("Matrix size index out of range"));
#endif
...
template <typename T> Matrix<T>& Matrix<T>::resize() { return clear(); }

template <typename T> Matrix<T>& Matrix<T>::resize(int size0, int size1) {
  m_store.resize(set_sizes(size0, size1));
  return *this;
}

template <typename T> Matrix<T>& Matrix<T>::resize(uint size0, uint size1) {
  m_store.resize(set_sizes(size0, size1));
  return *this;
}
...
public:
  ...
  operator+=(const LowerTriangularMatrix<T>& incoming);
  LowerTriangularMatrix<T>&
  operator-=(const LowerTriangularMatrix<T>& incoming);
...
unsigned int LowerTriangularMatrix<T>::set_size(int insize) {
  if (insize < 0)
    throw(std::invalid_argument("Invalid LowerTriangularMatrix size"));
  m_size = insize;
...
LowerTriangularMatrix<T>::LowerTriangularMatrix(
    const LowerTriangularMatrix<T>& incoming)
...
template <typename T>
inline LowerTriangularMatrix<T>&
LowerTriangularMatrix<T>::operator=(const T& val) {
  for (uint i = 0; i < m_store.size(); ++i)
    m_store[i] = val;
  return *this;
...
#ifdef SAFETY_FLAG
  if ((i0 < 0) || (i0 >= int(m_size)))
    throw(
        std::invalid_argument("LowerTriangularMatrix: 1st index out of range"));
  if ((i1 < 0) || (i1 >= int(m_size)))
    throw(
        std::invalid_argument("LowerTriangularMatrix: 2nd index out of range"));
  if (i0 < i1)
    throw(std::invalid_argument(
        "LowerTriangularMatrix: 2nd index cannot exceed 1st index"));
#endif
  return (i0 * (i0 + 1)) / 2 + i1;
}

template <typename T>
inline unsigned int LowerTriangularMatrix<T>::get_index(uint i0,
                                                        uint i1) const {
#ifdef SAFETY_FLAG
  if (i0 >= m_size)
    throw(
...
template <typename T>
inline unsigned int
LowerTriangularMatrix<T>::get_index(const std::vector<int>& ind) const {
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument("LowerTriangularMatrix: improper rank of "
                                "LowerTriangularMatrix indices"));
#endif
  return get_index(ind[0], ind[1]);
}
...
template <typename T>
inline unsigned int
LowerTriangularMatrix<T>::get_index(const std::vector<uint>& ind) const {
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument("LowerTriangularMatrix: improper rank of "
                                "LowerTriangularMatrix indices"));
...
template <typename T>
inline const T&
LowerTriangularMatrix<T>::operator()(const std::vector<int>& ind) const {
  return m_store[get_index(ind)];
}

template <typename T>
inline const T&
LowerTriangularMatrix<T>::operator()(const std::vector<uint>& ind) const {
  return m_store[get_index(ind)];
}
...
template <typename T>
inline void LowerTriangularMatrix<T>::put(uint i0, uint i1,
                                          const T& inputvalue) {
  m_store[get_index(i0, i1)] = inputvalue;
}

template <typename T>
inline void LowerTriangularMatrix<T>::put(const std::vector<int>& ind,
                                          const T& inputvalue) {
  m_store[get_index(ind)] = inputvalue;
}

template <typename T>
inline void LowerTriangularMatrix<T>::put(const std::vector<uint>& ind,
                                          const T& inputvalue) {
  m_store[get_index(ind)] = inputvalue;
}
...
template <typename T>
inline const T&
LowerTriangularMatrix<T>::get(const std::vector<int>& ind) const {
  return m_store[get_index(ind)];
}

template <typename T>
inline const T&
LowerTriangularMatrix<T>::get(const std::vector<uint>& ind) const {
  return m_store[get_index(ind)];
}
...
public:
  ...
  const double operator()(int i0, uint i1) const {
    return operator()(i0, int(i1));
  }
  const double operator()(uint i0, int i1) const {
    return operator()(int(i0), i1);
  }
  ...
  void modifyEigenvalues(std::complex<double> (*func)(double),
                         Matrix<std::complex<double>>& out_matrix) const;
...
inline unsigned int RealSymmetricMatrix::set_size(int insize) {
  if (insize < 0)
    throw(std::invalid_argument("Invalid RealSymmetricMatrix size"));
  m_size = insize;
  return (m_size * (m_size + 1)) / 2;
}
...
#ifdef SAFETY_FLAG
  if ((i0 < 0) || (i0 >= int(m_size)))
    throw(std::invalid_argument("RealSymmetricMatrix: 1st index out of range"));
  if ((i1 < 0) || (i1 >= int(m_size)))
    throw(std::invalid_argument("RealSymmetricMatrix: 2nd index out of range"));
#endif
...
inline unsigned int
RealSymmetricMatrix::get_index(const std::vector<int>& ind) const {
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument(
        "RealSymmetricMatrix: improper rank of RealSymmetricMatrix indices"));
#endif
  return get_index(ind[0], ind[1]);
}

inline unsigned int
RealSymmetricMatrix::get_index(const std::vector<uint>& ind) const {
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument(
        "RealSymmetricMatrix: improper rank of RealSymmetricMatrix indices"));
...
inline const double
RealSymmetricMatrix::operator()(const std::vector<int>& ind) const {
  return m_store[get_index(ind)];
}

inline const double
RealSymmetricMatrix::operator()(const std::vector<uint>& ind) const {
  return m_store[get_index(ind)];
}
...
inline void RealSymmetricMatrix::put(const std::vector<int>& ind,
                                     double inputvalue) {
  m_store[get_index(ind)] = inputvalue;
}

inline void RealSymmetricMatrix::put(const std::vector<uint>& ind,
                                     double inputvalue) {
  m_store[get_index(ind)] = inputvalue;
}
...
inline const double
RealSymmetricMatrix::get(const std::vector<int>& ind) const {
  return m_store[get_index(ind)];
}

inline const double
RealSymmetricMatrix::get(const std::vector<uint>& ind) const {
  return m_store[get_index(ind)];
}
...
public:
  ...
  const std::complex<double> operator()(uint i0, int i1) const {
    return operator()(int(i0), i1);
  }
  const std::complex<double> operator()(const std::vector<int>& ind) const;
  const std::complex<double>
  operator()(const std::vector<unsigned int>& ind) const;

  void put(int i0, int i1, const std::complex<double>& inputvalue);
  void put(uint i0, uint i1, const std::complex<double>& inputvalue);
  void put(int i0, uint i1, const std::complex<double>& inputvalue) {
    put(i0, int(i1), inputvalue);
  }
  void put(uint i0, int i1, const std::complex<double>& inputvalue) {
    put(int(i0), i1, inputvalue);
  }
  void put(const std::vector<int>& ind, const std::complex<double>& inputvalue);
  void put(const std::vector<unsigned int>& ind,
           const std::complex<double>& inputvalue);

  const std::complex<double> get(int i0, int i1) const;
  const std::complex<double> get(uint i0, uint i1) const;
  const std::complex<double> get(int i0, uint i1) const {
    return get(i0, int(i1));
  }
  const std::complex<double> get(uint i0, int i1) const {
    return get(int(i0), i1);
  }
...
inline unsigned int ComplexHermitianMatrix::set_size(int insize) {
  if (insize < 0)
    throw(std::invalid_argument("Invalid ComplexHermitianMatrix size"));
  m_size = insize;
  return (m_size * (m_size + 1)) / 2;
}
...
inline unsigned int ComplexHermitianMatrix::get_index(int i0, int i1) const {
  ...
  if ((i0 < 0) || (i0 >= int(m_size)))
    throw(std::invalid_argument(
        "ComplexHermitianMatrix: 1st index out of range"));
  if ((i1 < 0) || (i1 >= int(m_size)))
    throw(std::invalid_argument(
        "ComplexHermitianMatrix: 2nd index out of range"));
#endif
  return (i1 * (i1 + 1)) / 2 + i0;
}
...
ComplexHermitianMatrix::operator()(int i0, int i1) const {
  return get(i0, i1);
}

inline const std::complex<double>
ComplexHermitianMatrix::operator()(uint i0, uint i1) const {
  return get(i0, i1);
}

inline const std::complex<double>
ComplexHermitianMatrix::operator()(const std::vector<int>& ind) const {
  return get(ind);
}

inline const std::complex<double>
ComplexHermitianMatrix::operator()(const std::vector<uint>& ind) const {
  return get(ind);
}

inline void
ComplexHermitianMatrix::put(int i0, int i1,
                            const std::complex<double>& inputvalue) {
  if ((i0 == i1) && (inputvalue.imag() != 0.0))
    throw(std::invalid_argument("ComplexHermitianMatrix: nonzero imaginary "
                                "part in diagonal element assignment"));
  if (i0 <= i1)
    m_store[get_index(i0, i1)] = inputvalue;
  else
...
inline void
ComplexHermitianMatrix::put(uint i0, uint i1,
                            const std::complex<double>& inputvalue) {
  if ((i0 == i1) && (inputvalue.imag() != 0.0))
    throw(std::invalid_argument("ComplexHermitianMatrix: nonzero imaginary "
                                "part in diagonal element assignment"));
...
inline void
ComplexHermitianMatrix::put(const std::vector<uint>& ind,
                            const std::complex<double>& inputvalue) {
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument("ComplexHermitianMatrix: improper rank of "
                                "ComplexHermitianMatrix indices"));
#endif
  put(ind[0], ind[1], inputvalue);
}

inline void
ComplexHermitianMatrix::put(const std::vector<int>& ind,
                            const std::complex<double>& inputvalue) {
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument("ComplexHermitianMatrix: improper rank of "
...
inline const std::complex<double> ComplexHermitianMatrix::get(int i0,
                                                              int i1) const {
  return (i0 <= i1) ? m_store[get_index(i0, i1)]
                    : conjugate(m_store[get_index(i1, i0)]);
}

inline const std::complex<double> ComplexHermitianMatrix::get(uint i0,
                                                              uint i1) const {
  return (i0 <= i1) ? m_store[get_index(i0, i1)]
                    : conjugate(m_store[get_index(i1, i0)]);
}
...
inline const std::complex<double>
ComplexHermitianMatrix::get(const std::vector<int>& ind) const {
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument("ComplexHermitianMatrix: improper rank of "
                                "ComplexHermitianMatrix indices"));
...
inline const std::complex<double>
ComplexHermitianMatrix::get(const std::vector<uint>& ind) const {
#ifdef SAFETY_FLAG
  if (ind.size() != 2)
    throw(std::invalid_argument("ComplexHermitianMatrix: improper rank of "
                                "ComplexHermitianMatrix indices"));
...
typedef RealSymmetricMatrix HermMatrix;
typedef Matrix<double> TransMatrix;
typedef Vector<double> SVector;
