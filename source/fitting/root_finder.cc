...
inline bool isValidBracket(double f_left, double f_right, double zero_tol) {
  // Allow for numerical precision: if either value is very small, consider it
  // valid
  const double precision_factor = 1e-12;
  double threshold = zero_tol * precision_factor;

  return (f_left * f_right <= 0) || (std::abs(f_left) < threshold) ||
...
bool AdaptiveBracketRootFinder::findRoots(double a, double b,
  ...
  for (auto [lft, rgt] : brackets) {
    ...
    while (!stack.empty()) {
      ...
      if (fl * fim <= 0)
        stack.emplace_back(l, mid);
      if (fim * fr <= 0)
        stack.emplace_back(mid, r);
    }
  }
  ...
  if (roots.size() > 1) {
    ...
    auto new_end =
        std::unique(roots.begin(), roots.end(), [this](double a, double b) {
